        ...
        <div class="col-xs-12 col-md-6 col-md-offset-3">
            <div class="box box-primary">
                <div class="box-body">
                    ...
                    <div class="spacer"></div>
                    <div class="spacer"></div>
                    {{-- Razorpay (Inde) supprimé : non pertinent au Cameroun --}}
                    {{-- Stripe désactivé par défaut : aucune ressource externe ne doit être chargée. --}}
                    {{-- Pour le réactiver, retirer le @if(false) ci-dessous ET réintroduire        --}}
                    {{-- une intégration Stripe.js servie en local (vendor-local/) ou utiliser      --}}
                    {{-- Stripe Elements avec clé publique configurée via les paramètres POS.       --}}
                    @if(false && !empty($pos_settings['stripe_public_key']) && !empty($pos_settings['stripe_secret_key']))
                        {{-- Bloc Stripe désactivé : nécessite ressource externe checkout.stripe.com --}}
                    @endif
                    @else
                        <table class="table no-border">
                            <tr>
