...
class Helpers
{
    ...
    public static function build_url($protocol, $host, $base_path, $url)
    {
        ...
        if ($is_local_path) {
            ...
            if ($filepath === false) {
				// Formidable: fix URL with spaces or special characters.
				$filepath = realpath( urldecode( $ret ) );
				if ( false === $filepath ) {
					return null;
				}
				// End Formidable.
    ...
    public static function getFileContent($uri, $context = null, $offset = 0, $maxlen = null)
    {
		...
		// Formidable: some URLs shouldn't be run through Helpers::encodeURI().
	    $should_encode = \FrmPdfsAppHelper::dompdf_should_encode_uri( $uri );

        set_error_handler([self::class, 'record_warnings']);

        try {
            if ($is_local_path || ini_get('allow_url_fopen') || !$can_use_curl) {
                if ($is_local_path === false && $should_encode) {
