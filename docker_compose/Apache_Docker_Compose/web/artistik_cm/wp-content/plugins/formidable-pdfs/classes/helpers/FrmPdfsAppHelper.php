...
class FrmPdfsAppHelper {
	 ...
	 * @var string
	 */
	public static $plug_version = '2.0.6';
	...
	/**
	 * Checks if is saved to run this plugin.
	 *
	 * @since 2.0.6
	 *
	 * @return bool
...
HTML;
	...
	public static function get_svg_searches_and_replaces( &$searches, &$replaces, $html ) {
		// Get all <svg> tags.
		preg_match_all( '/<svg.*?>.*?<\/svg>/ms', $html, $matches, PREG_SET_ORDER );
	...
	private static function process_svg_content( &$svg ) {
		...
		// Remove width and height attributes, which cause styling issue for file type icons (issue #143).
