...
class frmBtsModBlockController {
	...
	/**
	 * Add an alert message to the form for enabling submit with AJAX.
	 *
	 * @since 3.0.3
	 *
	 * @param array $attributes Array containing 'formId', 'title', 'description', 'minimize', and 'className'.
	 * @return void
	 */
	public static function add_ajax_submit_alert_to_form( $attributes ) {
		$context = FrmAppHelper::get_simple_request(
			array(
				'type'  => 'request',
				'param' => 'context',
			)
		...
		if ( 'edit' !== $context || FrmForm::is_ajax_on( FrmForm::getOne( $attributes['formId'] ) ) ) {
			return;
		}
		?>
		<div class="frm-block-info">
			<span class="frm-block-info__title"><?php esc_html_e( 'Improve Your Form\'s User Experience', 'frmmodal' ); ?></span>
			<span>
				<?php esc_html_e( 'Enable AJAX to enhance the experience with modal forms. Go to the form settings and turn on "Submit this form with AJAX".', 'frmmodal' ); ?>
			</span>

			<p>
				<a href="#" class="button button-secondary frm-block-info__close-button" role="button">
					<?php esc_html_e( 'Close', 'formidable' ); ?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=formidable&frm_action=settings&id=' . absint( $attributes['formId'] ) ) ); ?>" class="button button-primary" role="button" target="_blank" rel="noopener">
