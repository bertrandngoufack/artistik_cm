...
class FrmProStatisticsController {
	...
	/**
	 * This is set in pre_stats_shortcode.
	 * It's the array of shortcode data.
	 * $m[3] is checked in order to determine if the legacy format is being used.
	 *
	 * @since 6.9.1
	 *
	 * @var array|null
	...
	/**
	 * This is triggered before stats_shortcode is.
	 * This is used to get access to $m, which is not passed when stats_shortcode is called.
	 * We need to know the value of $m[3] in order to know if the legacy filtering is being used.
	 *
	 * @since 6.9.1
	 *
	...
	public static function stats_shortcode( $atts ) {
		self::prepare_shortcode_atts( $atts );
	...
	public static function get_field_matches( $args ) {
		...
		if ( $filter_args['after_where'] && ! $filter_args['entry_ids'] ) {
			return array();
		}
	...
	/**
	 * Check if the legacy format is being used.
	 * This looks like [frm-stats id=1 type=count "2024-01-01"&lt;created_at&lt;"2024-12-31"].
	 * If we can detect a new filter like 1="Value" then return false.
	 *
	 * @since 6.9.1
	 *
	...
	/**
	 * Check if meta values for a field include non-numeric data.
	 *
	 * @since 6.8.4
	 *
	 * @param object $field
	...
	public static function has_non_numeric_values( $field, $args ) {
		self::prepare_shortcode_atts( $args );
		$meta_values = self::get_meta_values_for_single_field( $field, $args );
