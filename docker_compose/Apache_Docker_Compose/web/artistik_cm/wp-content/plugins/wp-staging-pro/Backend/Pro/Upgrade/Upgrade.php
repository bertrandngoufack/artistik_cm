...
class Upgrade
{
    ...
    private function upgrade5_0_5()
    {
        ...
        if (is_admin() && current_user_can('activate_plugins') && !empty($user->user_email)) {
            update_option(Notifications::OPTION_BACKUP_SCHEDULE_REPORT_EMAIL, $user->user_email);
        }
    ...
    private function upgradeCopyExcludedTablesToTablePushSelection()
    {
        ...
        if (update_option(Sites::STAGING_SITES_OPTION, $sites, false)) {
    ...
    /**
     * Disable Remote Sync by default for users who never used it.
     * Users with an existing API token have actively configured the feature, so keep it enabled.
     * @return void
     */
    private function upgrade6_7_1()
    {
        if (version_compare($this->previousVersion, '6.7.1', '>')) {
            return;
        }

        $existingEnabledOption = get_option('wpstg_remote_sync_enabled', null);

        // If user has explicitly set the enabled option, respect their choice
        if ($existingEnabledOption !== null) {
            return;
        }

        $apiToken = get_option('wpstg_remote_sync_api_token', false);

        // User has an API token (feature was used) -> explicitly enable
        if (!empty($apiToken)) {
            update_option('wpstg_remote_sync_enabled', '1', false);
            return;
        }

        // No token, no explicit setting -> explicitly disable
        update_option('wpstg_remote_sync_enabled', '0', false);
    }

    /**
     * @return void
     */
    private function maybeUpdateOptimizer()
    {
        if ($this->previousVersion === false || version_compare($this->previousVersion, WPStaging::getVersion(), '<')) {
