...
class Processing extends Job
{
    ...
    /** @var string */
    const FILTER_PUSH_EXCLUDED_FILES = 'wpstg_push_excluded_files';
    ...
    private function init()
    {
        ...
        if (isset($_POST["excludedDirectories"]) && is_array($_POST["excludedDirectories"])) {
            $this->options->excludedDirectories = $this->sanitize->sanitizeArrayString($_POST["excludedDirectories"]);
        }

        // Included Directories POST
        if (isset($_POST["includedDirectories"]) && is_array($_POST["includedDirectories"])) {
            $this->options->includedDirectories = $this->sanitize->sanitizeArrayString($_POST["includedDirectories"]);
        }

        // Extra Directories POST
        if (isset($_POST["extraDirectories"]) && !empty($_POST["extraDirectories"])) {
            if (is_array($_POST["extraDirectories"])) {
                $this->options->extraDirectories = $this->sanitize->sanitizeArrayString($_POST["extraDirectories"]);
            } else {
                $this->options->extraDirectories = explode(
                    ScanConst::DIRECTORIES_SEPARATOR,
                    $this->sanitize->sanitizeString($_POST["extraDirectories"])
                );
            }
        }
        ...
        $excludedDirectories = [
            ...
            $this->options->path . 'wp-content/wp-staging',
            $this->options->path . 'wp-content/wp-staging-sites',
            $this->options->path . 'wp-content/uploads/wp-staging-sites',
        ];
        ...
        // Excluded Files
        $this->options->excludedFiles = Hooks::applyFilters(self::FILTER_PUSH_EXCLUDED_FILES, [
            '.htaccess',
            '.DS_Store',
            '*.git',
            '*.svn',
    ...
    private function saveExcludedDirectories()
    {
        ...
        if (update_option(Sites::STAGING_SITES_OPTION, $this->options->existingClones, false) === false) {
            return false;
        }
    ...
    private function writePushLogHeader()
    {
        ...
        $this->addJobSettingsToLogs("Push");
    }
}
