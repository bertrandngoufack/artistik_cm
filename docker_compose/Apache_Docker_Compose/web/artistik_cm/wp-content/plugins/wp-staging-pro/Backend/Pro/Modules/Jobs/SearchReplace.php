...
use WPStaging\Backend\Modules\Jobs\SearchReplace as JobsSearchReplace;
...
class SearchReplace extends JobExecutable
{
    ...
    /** @var string */
    const FILTER_PUSH_SEARCHREPLACE_PARAMS = 'wpstg_push_searchreplace_params';
    ...
    protected function execute()
    {
        ...
        // Table is excluded
        $tablesExcluded = is_object($this->options) && property_exists($this->options, 'tablesExcludedByFilter') ? $this->options->tablesExcludedByFilter : [];
    ...
    private function searchReplace($table, $args)
    {
        ...
        $args = Hooks::applyFilters(self::FILTER_PUSH_SEARCHREPLACE_PARAMS, $args);
        ...
        $filter = apply_filters(JobsSearchReplace::FILTER_CLONE_SEARCH_REPLACE_EXCLUDED_ROWS, $filter);
        ...
        foreach ($data as $row) {
            ...
            foreach ($columns as $column) {
                ...
                if (is_object($this->options) && property_exists($this->options, 'wpBakeryActive')) {
