...
class Files extends JobExecutable
{
    ...
    /**
     * The visibility should be at least 'protected' to allow unit test 'FilesTest' to overwrite it.
     *
     * @var int
     */
    protected $maxFilesPerRun;
    ...
    public function initialize()
    {
        ...
        $this->wpDefaultDirectories = WPStaging::make(WpDefaultDirectories::class);
    ...
    protected function execute()
    {
        if ($this->isFinished()) {
            ...
            $pluginsCopier = WPStaging::make(PluginsCopier::class);
            $themesCopier  = WPStaging::make(ThemesCopier::class);
    ...
    private function getFilesAndCopy()
    {
        ...
        if (isset($this->options->copiedFiles) && $this->options->copiedFiles != 0) {
            $this->file->seek($this->options->copiedFiles + 1); // '+1' To make sure all files are copied.
        } else {
            // When running first time, setting seek(1) instead of seek(0) because first line is PHP_HEADER that is used for security purpose.
            $this->file->seek(1);
        }
    ...
    {
        $identifier = $this->pathAdapter->getIdentifierFromPath($file);
        $file       = $this->pathAdapter->getPathWithoutIdentifier($file);
        $filePath   = $this->getStagingPathByIdentifier($identifier, $file);
        ...
        if (($destination = $this->getDestination($identifier, $file)) === false) {
        ...
        try {
            if (is_dir($fullPath)) {
                $this->filesystem->copy($fullPath, $destination);
            } else {
                $this->filesystem->copyFile($fullPath, $destination);
            }
        ...
        if (!$this->isMultisiteAndPro()) {
            ...
            /**
             * Set parent directory permissions, @see function docblock
             */
     ...
     *
     * WARNING: This method applies chmod to dirname($file), not to $file itself. And dirname($file) returns the parent directory of the file/directory given.
     *
     * @param string $file Path to a file whose parent directory permissions should be set.
     * @return bool
     */
    ...
    /**
     * Gets staging path by identifier
     * @param string $identifier
     * @param string $file
     * @return bool|string
     */
    private function getStagingPathByIdentifier($identifier, $file)
    {
        if ($this->siteInfo->isWpContentOutsideAbspath()) {
            $source = trailingslashit($this->stagingPath) . ltrim($this->wpDefaultDirectories->getDefaultDirectoryByIdentifier($identifier), '/');

            return trailingslashit($source) . $file;
        }

        $fullPath = $this->pathAdapter->transformIdentifiableToPath($identifier . $file);

        return trailingslashit($this->stagingPath) . str_replace(trailingslashit(wp_normalize_path(ABSPATH)), '', wp_normalize_path($fullPath));
    }
     ...
     * @return bool|string
     */
    private function getDestination($identifier, $file)
    {
        $destinationPath = $this->pathAdapter->transformIdentifiableToPath($identifier . $file);
