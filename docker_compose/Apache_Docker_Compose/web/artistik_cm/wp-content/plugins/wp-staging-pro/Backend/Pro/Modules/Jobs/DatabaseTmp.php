...
class DatabaseTmp extends CloningProcess
{
     ...
     */
    const TMP_PREFIX_CUSTOM_TABLE = 'wpstgtmx_';

    /** @var string */
    const TMP_PREFIX_TO_DROP = 'wpstgbak_';
    ...
    protected function execute()
    {
        ...
        if (in_array($this->options->tables[$this->options->currentStep], $tablesExcluded)) {
    ...
    private function copyTable($srcTableName)
    {
        if ($this->shouldReplaceTablePrefix($srcTableName, $this->options->prefix)) {
            $destTableName = self::TMP_PREFIX . $this->strings->str_replace_first($this->options->prefix, '', $srcTableName);
        } else {
            $destTableName = self::TMP_PREFIX_CUSTOM_TABLE . $srcTableName;
        }
    ...
    private function startJobExternal($destTableName, $srcTableName)
    {
        ...
        // Handle case where getCreateStatement returns empty array on error
        if ($sql === []) {
            return false;
        }
