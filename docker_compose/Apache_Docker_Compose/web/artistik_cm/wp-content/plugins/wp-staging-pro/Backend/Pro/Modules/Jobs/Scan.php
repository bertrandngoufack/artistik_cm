...
class Scan extends Job
{
    ...
    protected function getDb()
    {
        ...
        if (
            ...
            $this->options->databaseUser     = $this->getValueFromArray('databaseUser', $currentClone);
            $this->options->databasePassword = $this->getValueFromArray('databasePassword', $currentClone);
            $this->options->databaseDatabase = $this->getValueFromArray('databaseDatabase', $currentClone);
            $this->options->databaseServer   = $this->getValueFromArray('databaseServer', $currentClone);
            $this->options->databasePrefix   = $this->getValueFromArray('databasePrefix', $currentClone);
            $this->options->databaseSsl      = $this->getValueFromArray('databaseSsl', $currentClone, false);
    ...
    {
        $excludedTables = [];
        $excludedTables = apply_filters(TableScanner::FILTER_PUSH_EXCLUDED_TABLES, $excludedTables);
    ...
    public function directoryListing($directories = null, $forceDisabled = false)
    {
        ...
        foreach ($directories as $name => $directory) {
            ...
            if (!empty($directory)) {
                $attributes['classes'] .= ' wpstg-parent-dir';
            }
            ...
            if (is_link($data['path'])) {
                ...
                $output .= "<span class='wpstg--tooltip wpstg--tooltip-normal'>
                    <img class='wpstg--dashicons wpstg--grey wpstg-symlink-dir-tooltip' src='{$assetsUrl}assets/svg/info-outline.svg' alt='info'/>
    ...
    protected function buildDirectories()
    {
        ...
        if (!file_exists($wpcontentDir)) {
            wp_die(sprintf(
                ...
                Language::localizeSupportUrl('https://wp-staging.com/support/')
            ));
        }
        ...
        foreach ($directories as $directory) {
            //Not a valid directory. Continue iteration but do not loop through the current further and look for subdirectories
