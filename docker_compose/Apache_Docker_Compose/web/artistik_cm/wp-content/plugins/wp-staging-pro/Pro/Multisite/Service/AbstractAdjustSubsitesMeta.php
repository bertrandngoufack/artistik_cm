...
namespace WPStaging\Pro\Multisite\Service;
 ...
 * Source site and destination site change depending upon the context.
 * But usually the source site is the from which we takes data, and do adjustments
 * and destination site is the one where we store this adjusted data.
 */
abstract class AbstractAdjustSubsitesMeta
{
    ...
    /** @var bool */
    protected $isSourceSubdomainInstall = false;
    ...
    public function getAdjustedSubsites(string $baseDomain, string $basePath, string $siteURL, string $homeURL, bool $isSubdomainInstall): array
    {
        ...
        $filterToUse           = $this->getFilterToUse();
        $filteredAdjustedSites = $this->applyFilters($filterToUse, $adjustedSites, $baseDomain, $basePath, $siteURL, $homeURL, $isSubdomainInstall);
        if (is_array($filteredAdjustedSites)) {
            return $filteredAdjustedSites;
        }

        $this->debugLog("Filter: {$filterToUse} does not return an array. Using default subsites.");
    ...
    private function adjustSubsite(SubsiteDto $site, string $baseDomain, string $basePath, string $siteURL, string $homeURL, bool $isSubdomainInstall): AdjustedSubsiteDto
    {
        ...
        $sourceSiteUrlWithoutScheme      = $this->getUrlWithoutScheme($this->sourceSiteUrl);
        $sourceHomeUrlWithoutScheme      = $this->getUrlWithoutScheme($this->sourceHomeUrl);
        $destinationSiteUrlWithoutScheme = $this->getUrlWithoutScheme($siteURL);
        $destinationHomeUrlWithoutScheme = $this->getUrlWithoutScheme($homeURL);
        ...
        $subsitePath                 = str_replace($this->trailingslashit($this->sourceSitePath), $basePath, $site->getPath());
        $subsiteSiteUrlWithoutScheme = str_replace($sourceSiteUrlWithoutScheme, $destinationSiteUrlWithoutScheme, $site->getSiteUrl());
        $subsiteSiteUrlWithoutScheme = $this->getUrlWithoutScheme($subsiteSiteUrlWithoutScheme);
        ...
        $subsiteHomeUrlWithoutScheme = $this->getUrlWithoutScheme($subsiteHomeUrlWithoutScheme);
        ...
        $baseSiteUrlWithoutScheme = $this->untrailingslashit($baseDomain . $basePath);
    ...
    protected function adjustDomainBasedSubsite(SubsiteDto $site, string $baseDomain, string $basePath, string $siteUrlSchemaPrefix, string $homeUrlSchemaPrefix, bool $isSubdomainInstall): AdjustedSubsiteDto
    {
        ...
        if (!$isSubdomainInstall) {
            $adjustedSite->setAdjustedDomain($baseDomain);
            $adjustedSite->setAdjustedPath($basePath . $this->trailingslashit($site->getDomain()));
        } else {
    ...
    protected function getAdjustedSubsiteInfo(string $subsiteDomain, string $subsitePath, string $baseSiteUrlWithoutScheme, string $subsiteUrlWithoutScheme, string $subsiteUrlWwwPrefix, bool $isSubdomainInstall)
    {
        ...
        if (!$isSubdomainInstall && ($subsiteName !== '')) {
            ...
            $subsiteName = empty($subsiteName) ? '' : $this->trailingslashit($subsiteName);
            $subsiteName = ltrim($subsiteName, '/');
            $subsitePath = $subsitePath . $subsiteName;
        }

        $subsiteUrlWithoutScheme = $this->untrailingslashit(rtrim($subsiteDomain, '/') . $subsitePath);
    ...
    abstract protected function getFilterToUse(): string;
}
