...
class NewAdminAccount extends DBCloningService
{
    protected function internalExecute()
    {
        ...
        if (!empty($result) && $result[0]->count > 0) {
            $query = "UPDATE `{$userTable}` SET `user_pass` = %s WHERE `user_email` = %s;";
            $query = $stagingDb->prepare($query, [
            ...
            $this->log(sprintf("Admin user '%s' password updated as the email already exists", $options->adminEmail));
            return true;
        }
