...
abstract class AbstractStorageTask extends AbstractTask
{
    ...
    public function execute(): TaskResponseDto
    {
        if ($this->remoteUploader->getError() !== false) {
            $this->logger->warning('There is error in storage task. Error message: ' . $this->remoteUploader->getError());
            return $this->generateResponse(false);
        }

        $chunkSize = Hooks::applyFilters(self::FILTER_REMOTE_STORAGES_CHUNK_SIZE, self::CHUNK_SIZE * MB_IN_BYTES);
    ...
    protected function upload(string $backupFilePath, string $fileName): TaskResponseDto
    {
        ...
        while (!$this->isThreshold()) {
            try {
                ...
                $uploaded = empty($this->jobDataDto->getRemoteStorageMeta()[$fileName]['Offset']) ? 0 : $this->jobDataDto->getRemoteStorageMeta()[$fileName]['Offset'];
    ...
    public function prepareBackupUpload(): bool
    {
        ...
        if (!$this->shouldCleanOldBackupsForRemoteUpload()) {
            $this->stepsDto->setTotal((int)$this->jobDataDto->getTotalBackupSize());
    ...
    private function cancelUpload(): TaskResponseDto
    {
        $this->remoteUploader->stopUpload();
        $this->jobDataDto->setEndTime(time());
        $this->logger->error($this->getStorageProvider() . ' - Upload Cancelled: Unable to upload backup');
        return $this->generateResponse(false);
    }
