...
use WPStaging\Framework\Traits\SlashTrait;

abstract class AbstractSearchReplacer implements DatabaseSearchReplacerInterface
{
    use SlashTrait;
    use ApplyFiltersTrait;

    /** @var string */
    const FILTER_CURRENT_SCHEME_SAME_SITE = 'wpstg.backup.restore.use_current_scheme_on_same_site';

    /** @var string */
    const FILTER_BACKUP_RESTORE_SEARCH_REPLACE = 'wpstg.backup.restore.searchreplace';

    /** @var string */
    const FILTER_BACKUP_RESTORE_EXTENDED_CSLASH_SEARCH_REPLACE = 'wpstg.backup.restore.extended-cslash-search-replace';
    ...
    /** @var bool */
    protected $matchingScheme = false;
    ...
    {
        $this->sourceSiteUrl = $this->untrailingslashit($sourceSiteUrl);
        $this->sourceHomeUrl = $this->untrailingslashit($sourceHomeUrl);
        $this->sourceSiteUploadURL = $this->untrailingslashit($sourceSiteUploadURL);
    }
    ...
    {
        if (empty($absPath) && defined('ABSPATH')) {
            $absPath = ABSPATH;
        }
        ...
        /** @var array */
        $searchReplaceToSort = $this->applyFilters(self::FILTER_BACKUP_RESTORE_SEARCH_REPLACE, $searchReplaceToSort, $absPath, $this->sourceSiteUrl, $this->sourceHomeUrl, $this->destinationSiteUrl, $this->destinationHomeUrl);
    ...
    public function buildHostname(string $url): string
    {
        ...
        if (array_key_exists('path', $parsedUrl)) {
            $hostname = $this->trailingslashit($hostname) . trim($parsedUrl['path'], '/');
        }
    ...
    {
        $this->sourceSiteHostname = $this->untrailingslashit($this->buildHostname($this->sourceSiteUrl));
        $this->sourceHomeHostname = $this->untrailingslashit($this->buildHostname($this->sourceHomeUrl));

        $this->destinationSiteUrl = $this->untrailingslashit($destinationSiteUrl);
        $this->destinationHomeUrl = $this->untrailingslashit($destinationHomeUrl);

        $this->destinationSiteHostname = $this->untrailingslashit($this->buildHostname($this->destinationSiteUrl));
        $this->destinationHomeHostname = $this->untrailingslashit($this->buildHostname($this->destinationHomeUrl));
    ...
    protected function replaceAbsPath(string $absPath)
    {
        ...
        // Normalized $absPath
        if ($this->normalizePath($this->sourceAbsPath) !== $this->sourceAbsPath) {
            ...
            $this->replace[] = $this->normalizePath($absPath);
            $this->replace[] = $this->normalizePath(addcslashes($absPath, '/'));
            $this->replace[] = $this->normalizePath(urlencode($absPath));

            if ($this->normalizePath(urlencode($this->sourceAbsPath)) !== $this->normalizePath(rawurlencode($this->sourceAbsPath))) {
                $this->search[] = $this->normalizePath(rawurlencode($this->sourceAbsPath));
                $this->replace[] = $this->normalizePath(rawurlencode($absPath));
            }
        }
    }
    ...
    protected function replaceUploadURLs()
    {
        ...
        $sourceUploadURLWithoutScheme = $this->trailingslashit($this->sourceSiteHostname) . $this->sourceSiteUploadURL;
        $destinationUploadURLWithoutScheme = $this->trailingslashit($this->destinationSiteHostname) . $this->destinationSiteUploadURL;
    ...
    protected function replaceURLs(string $sourceURL, string $destinationURL, bool $doubleSlashPrefix = true)
    {
        ...
        if (strpos($sourceURL, 'www.') === 0) {
            $this->search[]  = $prefix . substr($sourceURL, 4);
            $this->replace[] = $destinationGenericProtocol;
        }
    }
    ...
    {
        if ($this->isIdenticalSiteHostname() && !$this->isUseCurrentSchemeOnSameSite()) {
        ...
        if (strpos($this->sourceSiteHostname, 'www.') === 0) {
            $sourceSiteWithoutWWW = substr($this->sourceSiteHostname, 4);
            $this->search[]       = 'https://' . $sourceSiteWithoutWWW;
            $this->replace[]      = $this->destinationSiteUrl;
            $this->search[]       = 'http://' . $sourceSiteWithoutWWW;
            $this->replace[]      = $this->destinationSiteUrl;
        }
    ...
    protected function replaceMultipleHomeSchemes()
    {
        ...
        if ($this->isIdenticalHomeHostname() && !$this->isUseCurrentSchemeOnSameSite()) {
            return;
        }
    ...
    protected function replaceMultipleSchemesUploadURL()
    {
        ...
        $sourceUploadURLWithHttpsScheme = 'https://' . $this->trailingslashit($this->sourceSiteHostname) . $this->sourceSiteUploadURL;
        $destinationUploadURLWithScheme = $this->trailingslashit($this->destinationSiteUrl) . $this->destinationSiteUploadURL;
        $this->replaceURLs($sourceUploadURLWithHttpsScheme, $destinationUploadURLWithScheme, $doubleSlashPrefix = false);
        $sourceUploadURLWithHttpScheme = 'http://' . $this->trailingslashit($this->sourceSiteHostname) . $this->sourceSiteUploadURL;
    ...
    protected function isExtraCslashEscapingRequired(): bool
    {
        ...
        $this->requireCslashEscaping = $this->applyFilters(self::FILTER_BACKUP_RESTORE_EXTENDED_CSLASH_SEARCH_REPLACE, $requireCslashEscaping) === true;
    ...
    protected function isUseCurrentSchemeOnSameSite(): bool
    {
        return (bool)$this->applyFilters(self::FILTER_CURRENT_SCHEME_SAME_SITE, false);
    }
    ...
    protected function prepareUploadURLs()
    {
        if (empty($this->destinationSiteUploadURL)) {
            $this->destinationSiteUploadURL = $this->getUploadUrl();
        }

        $this->destinationSiteUploadURL = $this->untrailingslashit($this->destinationSiteUploadURL);

        $this->sourceSiteUploadURL = str_replace($this->trailingslashit($this->sourceSiteUrl), '', $this->sourceSiteUploadURL);
        $this->destinationSiteUploadURL = str_replace($this->trailingslashit($this->destinationSiteUrl), '', $this->destinationSiteUploadURL);
    }

    abstract protected function normalizePath(string $path): string;

    abstract protected function getUploadUrl(): string;
}
