...
{
    /** @var string */
    const FILTER_FRAMEWORK_SECURITY_DATA_ENCRYPTION = 'wpstg.framework.security.dataEncryption';

    /** @var string */
    const TRANSIENT_REDIRECT_URL = 'wpstg_redirect_url';
    ...
    public function saveOptions($options = [])
    {
        ...
        if (Hooks::applyFilters(self::FILTER_FRAMEWORK_SECURITY_DATA_ENCRYPTION, true)) {
    ...
    {
        if (empty($_GET['auth-storage']) || empty($_GET['sub-tab']) || empty($this->identifier) || strtolower(sanitize_text_field($_GET['sub-tab'])) !== strtolower($this->identifier) || !$this->isAuthenticated()) {
            return;
        }
    ...
    */
    ...
    {
        if (empty($_GET['sub-tab']) || empty($this->identifier) || strtolower(sanitize_text_field($_GET['sub-tab'])) !== strtolower($this->identifier)) {
            return;
        }
        ...
        $options = $this->getOptions();
        if (empty($options['isAuthenticated'])) {
            return;
        }

        $userDisplayName  = empty($options['userData']['displayName']) ? '' : $options['userData']['displayName'];
        $storageUsed      = empty($options['storageInfo']['used']) ? 0 : $options['storageInfo']['used'];
        $storageAllocated = empty($options['storageInfo']['allocation']['allocated']) ? 0 : $options['storageInfo']['allocation']['allocated'];
        $storageAvailable = (int)$storageAllocated - (int)$storageUsed;
        $storageAvailable = max(0, $storageAvailable); // to avoid negative cases, when the disk space is exceeded.
        ?>
        <div class="notice notice-success is-dismissible">
            <p>
                <?php printf(esc_html__('Name: %s. %s quota usage: %s used; %s available.', 'wp-staging'), esc_html($userDisplayName), esc_html($this->label), esc_html(size_format($storageUsed, 2)), esc_html(size_format($storageAvailable, 2))); ?>
            </p>
        </div>
        <?php
    }
    ...
    /**
     * Validates that a backup location path contains no spaces.
     * Sets $this->error and returns false if the location is invalid.
     *
     * @param string $location
     * @return bool
     */
    protected function isValidLocation(string $location): bool
    {
        if (empty($location)) {
            $this->error = esc_html__('Please enter folder path where backups will be stored.', 'wp-staging');
            return false;
        }

        if (preg_match('/\s/', $location)) {
            $this->error = esc_html__('Backup location must not contain whitespace.', 'wp-staging');
            return false;
        }
    ...
    public function isBusinessPlanOrHigher(): bool
    {
        ...
        return $licensing->isBusinessPlanOrHigher();
    }
