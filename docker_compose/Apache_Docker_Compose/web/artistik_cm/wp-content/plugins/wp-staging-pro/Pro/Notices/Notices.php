...
class Notices
{
    ...
    public function __construct(TransientCache $transientCache, BackupsFinder $backupsFinder, SiteInfo $siteInfo, PluginInfo $pluginInfo)
    {
        ...
        $this->proNoticesViewPath = WPSTG_VIEWS_DIR . "pro/notices/";
    ...
    public function renderNotices()
    {
        ...
        $this->showWpstgRestoreFileExistsInRootPath();
    ...
    public function getLicenseKeyInvalidNotice()
    {
        ...
        if ($this->showAllNotices || ((isset($this->license->error) && $this->license->error !== 'expired') || $this->license === false || (isset($this->license->license) && $this->license->license === false))) {
    ...
    /**
     * Show notice if wpstg-restore.php file exist in root path
     */
    private function showWpstgRestoreFileExistsInRootPath()
    {
        $filePath = ABSPATH . 'wpstg-restore.php';
        if (!file_exists($filePath)) {
            return;
        }

        if ($this->showAllNotices || current_user_can("update_plugins")) {
            require $this->proNoticesViewPath . "wpstg-restore-in-root-path.php";
        }
    }
}
