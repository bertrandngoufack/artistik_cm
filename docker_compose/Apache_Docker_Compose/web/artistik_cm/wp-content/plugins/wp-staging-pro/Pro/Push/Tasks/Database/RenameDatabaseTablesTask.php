...
class RenameDatabaseTablesTask extends PushTask
{
    /**
     * @var string
     */
    const HOOK_KEEP_OPTIONS = 'wpstg.push.keep_options';

    /**
     * @var string
     */
    const FILTER_EXCLUDE_TABLES_DURING_PUSH = 'wpstg.push.exclude.tables';
    ...
    {
        $this->tablesRenamer->setTmpPrefix($this->jobDataDto->getTmpPrefix());
        ...
        $this->tablesRenamer->setShortNamedTablesToRename($this->jobDataDto->getShortNamedTablesToRename());
        $this->tablesRenamer->setShortNamedTablesToDrop($this->jobDataDto->getShortNamedTablesToDrop());
        $this->tablesRenamer->setRenameViews(true);
        $this->tablesRenamer->setThresholdCallable([$this, 'isMaxExecutionThreshold']);
        // Tables to not push in the site
        $excludedTables = [Queue::QUEUE_TABLE_NAME];
        $excludedTables = array_merge($excludedTables, Hooks::applyFilters(self::FILTER_EXCLUDE_TABLES_DURING_PUSH, []));
    ...
    protected function keepOptions()
    {
        ...
        // Keep the original WP STAGING settings intact upon pushing.
        $this->optionsToKeep[] = [
            'name'     => 'wpstg_settings',
        ...
        // If this is a staging site, keep the staging site status after push.
        $this->optionsToKeep[] = [
    ...
    {
        $tmpPrefix = $this->jobDataDto->getTmpPrefix();
        $this->setupTableRenamer();
        $this->setCurrentTaskDto($this->tablesRenamer->setupRenamer());

        // Store some information to re-add after we push the database.
        ...
        if ($totalTablesToRename === 0) {
            $this->logger->critical('Could not find any database table to push. Dump seems to be corrupt. Contact support@wp-staging.com.');
            throw new Exception("Could not find any database table to push. Dump seems to be corrupt.");
        }

        $this->currentTaskDto->totalTablesToRename = $totalTablesToRename;
        $this->currentTaskDto->totalTablesRenamed  = 0;
        ...
        if (is_multisite() && !$this->isSubsitePush()) {
        ...
        $this->currentTaskDto->dataToPreserve = $dataToPreserve;
        ...
        $this->logger->info(sprintf('Found %d tables to rename.', $this->currentTaskDto->totalTablesToRename));
    }
    ...
    protected function performDatabaseRename(): bool
    {
        ...
        // If there are no non-conflicting tables renamed, we will stop the push process.
        $result = $this->tablesRenamer->renameNonConflictingTables();
        if ($result === false) {
            if ($this->tablesRenamer->getRenamedTables() === 0) {
                $this->logger->critical('Could not push non-conflicting tables. Contact support@wp-staging.com.');
                throw new Exception("Could not push non-conflicting tables.");
            }

            $this->currentTaskDto->nonConflictingTablesRenamed = $this->tablesRenamer->getNonConflictingTablesRenamed();
            $this->logger->info(sprintf('Renamed %d/%d tables.', $this->currentTaskDto->nonConflictingTablesRenamed, $this->currentTaskDto->totalTablesToRename));
            $this->setCurrentTaskDto($this->currentTaskDto);
            return false;
        }
        ...
        $this->logger->info(sprintf('Renamed %d/%d tables.', $tablesRenamed, $this->currentTaskDto->totalTablesToRename));
        $this->setCurrentTaskDto($this->currentTaskDto);
    ...
    /**
     * Executes actions after a database has been pushed.
     * @return void
     */
    protected function postDatabaseRenameActions()
    {
        ...
        $databaseData = $this->currentTaskDto->dataToPreserve;
        ...
        $this->logger->info('Database pushed successfully.');
    }
    ...
    protected function renameViewReferences()
    {
        ...
        foreach ($views as $view) {
            ...
            $query = str_replace($this->jobDataDto->getTmpPrefix(), $this->tableService->getDatabase()->getPrefix(), $query);
     ...
     * @return bool
     */
    protected function isSubsitePush(): bool
    {
        return false;
    }
