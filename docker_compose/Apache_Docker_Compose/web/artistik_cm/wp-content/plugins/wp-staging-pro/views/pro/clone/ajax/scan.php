...
$cloneAjaxViewPath = WPSTG_VIEWS_DIR . 'clone/ajax/';
...
$canShowWooCheckbox = in_array($licensePriceId, $acceptablePlans);
$cloneUrl           = '';
$isValidLicense     = WPStagingPro::isValidLicense();
$licensePlanName    = '';
if (empty($licenseData->license) || $licenseData->license === 'invalid') {
    $licensePlanName = '(Unregistered)';
}

if ((!empty($licenseData->license) && $licenseData->license === 'disabled') || (!empty($licenseData->error) && $licenseData->error === 'disabled')) {
    $licensePlanName = '(License Disabled)';
}

if ((!empty($licenseData->license) && $licenseData->license === 'expired') || (!empty($licenseData->error) && $licenseData->error === 'expired')) {
...
<label id="wpstg-clone-label" for="wpstg-new-clone-id">
    <input type="text" id="wpstg-new-clone-id" class="wpstg-input wpstg-input-lg"
        ...
        <?php if ($options->current !== null) {
            $cloneUrl = $options->currentClone['url'];
...
<div class="wpstg-tabs-wrapper">
    <?php if ($isValidLicense && is_multisite() && is_main_site()) : ?>
    ...
    <fieldset class="wpstg-tab-section" id="wpstg-network-options" style="display: block;">
        <?php require($this->viewsPath . 'pro/clone/ajax/network-options.php'); ?>
    </fieldset>
    <?php endif; ?>
    ...
    <fieldset class="wpstg-tab-section" id="wpstg-scanning-db">
        <?php require($this->viewsPath . 'selections/database-tables.php'); ?>
    </fieldset>
    ...
    <fieldset class="wpstg-tab-section" id="wpstg-scanning-files">
        <?php require($this->viewsPath . 'selections/files.php'); ?>
    </fieldset>

    <a href="javascript:void(0)" class="<?php echo $isValidLicense ? 'wpstg-tab-header' : 'wpstg-invalid-license-tab-header' ?>" data-id="#wpstg-advanced-settings" >
        <span class="wpstg-tab-triangle"></span>
        <?php
            echo esc_html__("Advanced Settings", "wp-staging");
        ?>
        <?php if (!$isValidLicense) : ?>
        <span class="wpstg--red-warning"><?php echo esc_html($licensePlanName); ?></span>
        <?php endif; ?>
    </a>
    ...
    <div class="wpstg-tab-section" id="wpstg-advanced-settings">
        ...
        if ($options->mainJob !== Job::UPDATE) {
            require_once($cloneAjaxViewPath . 'login-data.php');
            require_once($cloneAjaxViewPath . 'external-database.php');
            require_once($cloneAjaxViewPath . 'custom-directory.php');
            require_once($cloneAjaxViewPath . 'symlink-uploads.php');
        }

        if ($options->mainJob === Job::STAGING) {
            require_once($cloneAjaxViewPath . 'cron-setting.php');
        }

        require_once($cloneAjaxViewPath . 'mail-setting.php');
        require_once($cloneAjaxViewPath . 'plugins-update.php');

        if (!empty($canShowWooCheckbox)) {
            require_once($this->viewsPath . 'pro/clone/ajax/disable-woo-scheduler.php');
        }
