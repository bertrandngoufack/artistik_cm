...
class FrmChatAppController {
	...
	private static function remove_form_hooks() {
		...
		remove_filter( 'frm_form_object', array( __CLASS__, 'maybe_extract_title_and_description' ) );
	}
	...
	private static function init_chat_form( $form ) {
		...
		add_filter( 'frm_form_object', array( __CLASS__, 'maybe_extract_title_and_description' ) );
	...
	public static function add_field_container_class( $class_string ) {
		$class = 'frm_chat_form ';
		if ( ! empty( self::$form->options['chat_transition_type'] ) ) {
			$class .= 'frm_chat_' . self::$form->options['chat_transition_type'] . ' ';
		}
		return str_replace( 'class="', 'class="' . $class, $class_string );
	}
	...
	public static function add_form_object_filter( $atts ) {
		...
		add_filter(
			...
			function( $form ) use ( $form_id ) {
				if ( is_object( $form ) && $form_id === (int) $form->id ) {
	...
	public static function include_chat_css() {
		readfile( FrmChatAppHelper::path() . '/css/chat.css' );
	}
	...
	public static function form_usage_data( $data, $args ) {
		...
		if ( ! empty( $form->options['chat'] ) ) {
			$data['chat']                 = 1;
			$data['chat_include_arrows']  = ! empty( $form->options['chat_include_arrows'] ) ? 1 : 0;
			$data['chat_show_start_page'] = ! empty( $form->options['chat_show_start_page'] ) ? 1 : 0;
			$data['chat_progress_type']   = isset( $form->options['chat_progress_type'] ) ? $form->options['chat_progress_type'] : '';
		}
	...
	/**
	 * Always extract the title and description even when the form comes from another source.
	 * This helps to prevent issues where the form description appears twice when editing an entry.
	 *
	 * @since 1.1.5
	 *
	 * @param stdClass $form
	 * @return stdClass
	 */
	public static function maybe_extract_title_and_description( $form ) {
		if ( $form->id === self::$form->id ) {
			$helper = new FrmChatHtmlHelper( $form );
			$helper->extract_title_and_description();
		}
		return $form;
	}
}
