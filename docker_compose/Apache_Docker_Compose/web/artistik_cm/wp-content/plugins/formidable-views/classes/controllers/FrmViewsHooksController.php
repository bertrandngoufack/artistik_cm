...
class FrmViewsHooksController {
	 ...
	 * @return bool
	 */
	private static function should_init_views() {
		global $frm_vars;
		if ( empty( $frm_vars['pro_is_authorized'] ) ) {
			return false;
		}
		...
		return $is_free_installed && $is_pro_installed;
	}

	public static function load_hooks() {
		add_action( 'wp_before_admin_bar_render', 'FrmViewsAppController::admin_bar_configure', 25 );
		add_action( 'init', 'FrmViewsAppController::load_lang', 0 );
		...
		add_action( 'wp_footer', array( FrmViewsInlineStyleController::get_instance(), 'output_styles' ) );

		add_filter( 'the_content', 'FrmViewsDisplaysController::get_content', 10 );
		add_filter( 'the_content', 'FrmViewsDisplaysController::add_view_wrapper_class', 20 );
	...
	public static function load_admin_hooks() {
		...
		add_action( 'admin_footer', array( FrmViewsInlineStyleController::get_instance(), 'output_styles' ) );
	...
	public static function load_controllers( $controllers ) {
		_deprecated_function( __METHOD__, '5.9' );
		return $controllers;
	}
}
