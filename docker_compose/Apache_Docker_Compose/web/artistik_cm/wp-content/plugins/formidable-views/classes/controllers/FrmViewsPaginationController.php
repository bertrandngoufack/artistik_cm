...
class FrmViewsPaginationController {
	...
	public static function load_page() {
		...
		self::remove_after_view_filters();
	...
	private static function maybe_overwrite_view_link( $view ) {
		...
		add_filter(
			...
			function ( $permalink, $post ) use ( $view, $site_url ) {
				return $view->ID === $post->ID ? $site_url : $permalink;
			},
			10,
			2
		);
	...
	/**
	 * Prevent the Export to CSV and other things that get added
	 * below the pagination every time the view loads from running on AJAX pagination.
	 *
	 * @since 5.9
	 *
	 * @return void
	 */
	private static function remove_after_view_filters() {
		global $wp_filter;
		$hook_name = 'frm_after_display_content';

		if ( ! isset( $wp_filter[ $hook_name ] ) || ! ( $wp_filter[ $hook_name ] instanceof WP_Hook ) ) {
			return;
		}

		$wp_filter[ $hook_name ]->callbacks = array_filter(
