...
class FrmViewsDisplay {
	...
	public static function convert_table_option_key_to_label( $option ) {
		switch ( $option ) {
			case 'created_at':
				return __( 'Entry creation date', 'formidable' );
			case 'updated_at':
				return __( 'Entry updated date', 'formidable-views' );
			case 'id':
				return __( 'Entry ID', 'formidable' );
			case 'item_key':
				return __( 'Entry key', 'formidable-views' );
			case 'post_id':
				return __( 'Post ID', 'formidable' );
			case 'parent_item_id':
				return __( 'Parent Entry ID', 'formidable' );
			case 'is_draft':
	...
	public static function update( $id, $values ) {
		...
		$fields = array( 'dyncontent', 'type', 'show_count', 'form_id', 'entry_id', 'active_preview_filter', 'grid_view', 'table_view', 'view_type', 'custom_css' );
	...
	public static function get_auto_custom_display( $args ) {
		...
		if ( $args['post_id'] && ! $args['entry_id'] ) {
			...
			$args['entry_id'] = FrmDb::get_var( 'frm_items', array( 'post_id' => $args['post_id'] ) );
		}
	...
	private static function is_field_sort_option( $option ) {
		return is_numeric( $option )
			...
			/**
			 * Filters whether a sort option targets a field.
			 *
			 * @since 5.9
			 *
			 * @param bool   $is_field_sort_option Whether the option targets a field.
			 * @param string $option The sort option.
			 */
			|| apply_filters( 'frm_views_is_field_sort_option', false, $option );
	}
	...
	private static function prepare_ordered_entries_query( &$query, &$args, $o_key, $o_field, $first_order ) {
		...
		if ( ! empty( $o_field->field_options['post_field'] ) ) {
			...
			if ( in_array( $o_field->type, array( 'name', 'address' ), true ) ) {
				 ...
				 * @since 5.9
				 *
				 * @param bool   $handled Whether the order by field has been handled.
				 * @param array  $query The query array.
				 * @param array  $args The args array.
				 * @param string $o_key The order by key.
				 * @param object $o_field The order by field.
				 * @param string $order The order.
				 */
				$query_result = apply_filters( 'frm_views_order_by_field', false, $query, $args, $o_key, $o_field, $order );
