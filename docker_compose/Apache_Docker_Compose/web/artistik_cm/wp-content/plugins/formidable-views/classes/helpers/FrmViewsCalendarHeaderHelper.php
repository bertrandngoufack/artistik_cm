...
class FrmViewsCalendarHeaderHelper {
	...
	public function get_html() {
		...
		$rtl                = preg_match( '/frm_rtl/', FrmFormsHelper::get_form_style_class( $this->view ) );
		$wrapper_classname .= $rtl ? ' frm_rtl' : '';
		$wrapper_classname .= empty( $this->view->frm_calendar_event_popup ) ? ' frm-no-popup' : '';

		return '<div class="frmcal frm-modern-calendar ' . esc_attr( $wrapper_classname ) . '" id="frmcal-' . esc_attr( $this->view->ID ) . '" data-gutenberg-classname="' . esc_attr( FrmViewsCalendarHelper::gutenberg_wrapper_classname( $this->view->ID ) ) . '">' . $this->content();
	}
	...
	private function calendar_button_go_current_month( $active_month, $active_year ) {
		...
		return '<a class="frmcal-today-button frmcal-hide-on-mobile" href="' . esc_url(
			add_query_arg(
				array(
