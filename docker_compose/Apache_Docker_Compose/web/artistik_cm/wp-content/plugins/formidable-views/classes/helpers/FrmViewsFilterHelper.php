...
class FrmViewsFilterHelper {
	...
	public function update_where_based_on_view_filters( &$where ) {
		...
		for ( $index = 1; $index < $count; ++$index ) {
			...
			$key         = array_key_first( $where_by_group[ $index ] );
			...
			if ( isset( $where_index['it.id'] ) && 1 === count( $where_index['it.id'] ) ) {
				...
				if ( $or ) {
					...
					if ( $this->should_ignore_where_group( $where_index ) ) {
						continue;
					}
	...
	/**
	 * Do not bother adding a group to the query if there are no entries that match the group.
	 *
	 * @since 5.9
	 *
	 * @param array $where
	 * @return bool
	 */
	private function should_ignore_where_group( $where ) {
		return ( 1 === count( $where ) || empty( $where['or'] ) ) && isset( $where['it.id'] ) && ! $where['it.id'];
	}
	...
	private function get_where_for_filter_group( $where, $where_for_group ) {
		...
		foreach ( $where_for_group as $i => $where_field ) {
			...
			if ( $this->should_add_where_to_frm_items_query_and_continue( $where_field ) ) {
	 ...
	 * @return bool
	 */
	private function should_add_where_to_frm_items_query_and_continue( $where_field ) {
		$should_add_and_continue = ! is_numeric( $where_field ) && ! $this->is_name_subfield( $where_field );
	...
	private function prepare_where_is( $i ) {
		...
		if ( is_array( $view->frm_where_val[ $i ] ) && $view->frm_where_val[ $i ] ) {
	...
	public static function clear_empty_args_where( &$array ) {
		foreach ( $array as $key => &$value ) {
			if ( is_array( $value ) ) {
				if ( ! $value || ( 2 === count( $value ) && isset( $value['or'] ) && empty( $value[0] ) ) ) {
					unset( $array[ $key ] );
				} else {
