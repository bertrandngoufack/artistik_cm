...
class FrmViewsEditorController {
	...
	private static function register_view_editor_scripts_and_styles() {
		...
		if ( is_callable( 'FrmAppController::enqueue_components_scripts' ) ) {
			FrmAppController::enqueue_components_scripts();
		}

		self::register_custom_css_editor_js();
	...
	private static function register_custom_css_editor_js() {
		...
		wp_add_inline_script(
			'code-editor',
			'var frmSingleViewListingPageCustomCssEditor = wp.codeEditor.initialize( "frm-view-listing-page--custom-css", ' . wp_json_encode( $css_editor_settings ) . ' ), 
			frmSingleViewDetailPageCustomCssEditor = wp.codeEditor.initialize( "frm-view-detail-page--custom-css", ' . wp_json_encode( $css_editor_settings ) . ' );'
		);
	}
	...
	private static function get_data() {
		...
		$response_data = array(
			...
			'listingPageCustomCss' => self::remove_local_scope_from_css( self::get_listing_page_custom_css() ),
			'detailPageCustomCss'  => self::remove_local_scope_from_css( self::get_option_value( 'detail_page_custom_css' ) ),
		);
	...
	/**
	 * Get the listing page custom CSS, falling back to the legacy custom_css option.
	 *
	 * @return string
	 */
	private static function get_listing_page_custom_css() {
		$legacy_setting = self::get_option_value( 'custom_css' );
		return $legacy_setting ? $legacy_setting : self::get_option_value( 'listing_page_custom_css' );
	}
	...
	private static function get_view_name( $view = null ) {
		...
		if ( $view && false !== strpos( $view->post_title, __( 'Untitled', 'formidable' ) ) ) {
		...
		$post_query = array(
			'post_title LIKE%' => __( 'Untitled', 'formidable' ),
	...
	/** Nest CSS selectors under a local scope class.
	 *
	 * @since 5.10
	 *
	 * @param string $css
	 ...
	 * @return string
	 */
	private static function add_local_scope_to_css( $css ) {
		if ( ! class_exists( 'FrmCssScopeHelper' ) ) {
			return $css;
		}
		$css_scope_helper = new FrmCssScopeHelper();
		return $css_scope_helper->nest( $css, 'frm-view-content-' . self::$view_id );
	}

	/** Remove local scope nesting from CSS selectors.
	 *
	 * @since 5.10
	 *
	 * @param string $css
	 ...
	 * @return string
	 */
	private static function remove_local_scope_from_css( $css ) {
		if ( ! class_exists( 'FrmCssScopeHelper' ) ) {
			return $css;
		}
		$css_scope_helper = new FrmCssScopeHelper();
		return $css_scope_helper->unnest( $css, 'frm-view-content-' . self::$view_id );
	}
	...
	private static function update_view() {
		...
		$is_timeline_type        = FrmViewsDisplaysHelper::is_timeline_type( $view );
		$listing_page_custom_css = FrmAppHelper::get_param( 'listingPageCustomCss', '', 'post', 'sanitize_textarea_field' );
		$detail_page_custom_css  = FrmAppHelper::get_param( 'detailPageCustomCss', '', 'post', 'sanitize_textarea_field' );
		...
		$options = array(
			...
			'listing_page_custom_css' => self::add_local_scope_to_css( $listing_page_custom_css ),
			'detail_page_custom_css'  => self::add_local_scope_to_css( $detail_page_custom_css ),
		);
		...
		$data = array(
			...
			'custom_css'            => '', // Unset the legacy views CSS setting (replaced with listing_page_custom_css).
		);
	...
	private static function get_dropdown_options() {
		...
		if ( $views_by_form_id ) {
			...
			foreach ( $form_data as $row ) {
				$category  = ( $row->name ? $row->name : __( 'Untitled', 'formidable' ) ) . ' (' . __( 'Form', 'formidable' ) . ' ' . $row->id . ')';
				$options[] = array(
				...
				foreach ( $views_by_form_id[ $row->id ] as $view ) {
					$options[] = array(
						'id'       => $view->ID,
						'name'     => $view->post_title ? $view->post_title : __( 'Untitled', 'formidable' ),
