...
class FrmViewsDisplaysController {
	...
	public static function menu() {
		...
		add_submenu_page( '', 'Formidable | ' . __( 'Views', 'formidable' ), __( 'Views', 'formidable' ), 'frm_edit_displays', 'formidable-views-editor', 'FrmViewsEditorController::view_editor' );
	}
	...
	private static function maybe_output_style_to_hide_filter_button() {
		global $post_type;
		if ( self::$post_type === $post_type && FrmAppHelper::simple_get( 'form', 'absint' ) ) {
	...
	private static function render_new_button() {
		...
		<a <?php FrmAppHelper::array_to_html_params( $atts, true ); ?>>
			<?php
			FrmAppHelper::icon_by_class( 'frmfont frm_plus_icon frm_svg15' );
	...
	public static function post_row_actions( $actions, $post ) {
		...
		$actions['duplicate'] = '<a ' . FrmAppHelper::array_to_html_params( $atts ) . '>' . esc_html__( 'Duplicate', 'formidable' ) . '</a>';
	...
	public static function manage_columns( $columns ) {
		...
		$columns['actions']     = __( 'Actions', 'formidable' );
	...
	public static function manage_custom_columns( $column_name, $id ) {
		switch ( $column_name ) {
			...
			case 'old_id':
				...
				$val    = $old_id ? absint( $old_id ) : esc_html__( 'N/A', 'formidable' );
				break;
			case 'name':
				$post = get_post( $id );
			...
			case 'form_id':
				...
				break;
			case 'actions':
				$val = '<a href="#" class="frm-embed-view" role="button" aria-label="' . esc_html__( 'Embed View', 'formidable-views' ) . '">' . FrmAppHelper::icon_by_class( 'frmfont frm_code_icon', array( 'echo' => false ) ) . '</a>';
				$val = '<div>' . $val . '</div>';
				break;
			case 'application':
	...
	public static function mb_adv_info( $post ) {
		...
		FrmFormsController::mb_tags_box( $post->frm_form_id, '', FrmViewsAppHelper::views_path() . '/editor/mb_adv_info.php' );
	}
	...
	/**
	 * Add a wrapper class to a View when there is Custom CSS set.
	 * This way we can scope the View's styles so they only apply to the View.
	 *
	 * @since 5.10
	 *
	 ...
	 * @return string
	 */
	public static function add_view_wrapper_class( $content ) {
		global $post;

		if ( ! empty( $post->frm_custom_css ) ) {
			// For backward compatibility.
			$post->frm_listing_page_custom_css = $post->frm_custom_css;
		}

		if ( ! $post || ( empty( $post->frm_listing_page_custom_css ) && empty( $post->frm_detail_page_custom_css ) ) ) {
			return $content;
		}
		...
		return self::add_class_to_root_level_element( $content, 'frm-view-content-' . $post->ID );
	}

	/**
	 * Add class to all root-level elements (tags at start or after closing tags).
	 *
	 * @since 5.10
	 *
	 * @param string $content   The HTML content to modify.
	 * @param string $classname The class name to add to root-level elements.
	 * @return string The modified HTML content.
	 */
	private static function add_class_to_root_level_element( $content, $classname ) {
		$void_elements = array( 'area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr' );
		$depth         = 0;
		$offset        = 0;
		$result        = '';

		while ( preg_match( '/<(\/?)(\w+)([^>]*?)\s*(\/?)>/s', $content, $match, PREG_OFFSET_CAPTURE, $offset ) ) {
			$full_match      = $match[0][0];
			$match_pos       = $match[0][1];
			$is_closing      = '/' === $match[1][0];
			$tag_name        = strtolower( $match[2][0] );
			$is_self_closing = '/' === $match[4][0] || in_array( $tag_name, $void_elements, true );
			...
			if ( $is_closing ) {
				...
				$result .= self::add_class_to_tag( $full_match, $classname );
				if ( ! $is_self_closing ) {
					++$depth;
				}
			} else {
	...
	/**
	 * Add a class to a single HTML tag string.
	 *
	 * @since 5.10
	 *
	 * @param string $tag       The full HTML tag (e.g. '<div id="foo">').
	 * @param string $classname The class name to add.
	 * @return string The modified tag.
	 */
	private static function add_class_to_tag( $tag, $classname ) {
	...
	public static function build_calendar( $new_content, $entry_ids, $shortcodes, $view, $is_legacy = false ) {
		...
		$prev_year         = gmdate( 'Y', strtotime( '-1 month', $timestamp ) );
		...
		$prev_month_startday = $prev_month_maxday - ( 6 - ( 7 - ( $startday - $week_begins ) ) );
		$efield              = false;
		$field               = false;
		...
		while ( $next_set = array_splice( $entry_ids, 0, 30 ) ) {
			...
			foreach ( $entries as $entry ) {
				self::calendar_daily_entries( $entry, $view, compact( 'year', 'month', 'startday', 'maxday', 'prev_month', 'next_month', 'prev_month_maxday', 'prev_month_startday', 'prev_year', 'field', 'efield', 'time_field', 'is_legacy' ), $daily_entries );
			}
		}
	...
	public static function calendar_daily_entries( $entry, $display, $args, array &$daily_entries ) {
		...
		if ( is_numeric( $display->frm_date_field_id ) ) {
			...
			if ( $entry->post_id && ! $date && $args['field'] && ! empty( $args['field']->field_options['post_field'] ) ) {
		...
		for ( $i = 0; $i < $visible_days_count; $i++ ) {
			...
			$formatted_date = $day > 0
				...
				: gmdate( 'Y-m-d', strtotime( $args['prev_year'] . '-' . $args['prev_month'] . '-' . ( $args['prev_month_maxday'] + $day ) ) );
			...
			$multiday_event_dates = $repeating_period ? $start_multiday_dates : $dates;
			if ( $is_legacy ? in_array( $formatted_date, $dates, true ) : in_array( $formatted_date, $multiday_event_dates, true ) ) {
				if ( null !== $args['time_field'] ) {
					$entry->time = FrmEntryMeta::get_meta_value( $entry, $args['time_field'] );
				}

				$entry->event_length = self::get_event_length( $no_repeating_entry_dates, $args['year'], $args['month'], $args['prev_month_startday'], $repeating_period );
	 ...
	 */
	private static function get_event_length( $dates, $year, $month, $visible_prev_month_startday, $repeating_period ) {
		return count(
			array_unique(
				array_filter(
					$dates,
					function ( $date ) use ( $year, $month, $visible_prev_month_startday, $repeating_period ) {
						$date = FrmViewsCalendarHelper::maybe_fix_repeating_date( $date, compact( 'year', 'month' ), $repeating_period );
	...
	private static function get_repeating_dates( $entry, $display, $args, array &$dates ) {
		...
		$t_strings = array( __( 'day', 'formidable' ), __( 'days', 'formidable' ), __( 'daily', 'formidable-views' ), __( 'week', 'formidable' ), __( 'weeks', 'formidable' ), __( 'weekly', 'formidable-views' ), __( 'month', 'formidable' ), __( 'months', 'formidable' ), __( 'monthly', 'formidable-views' ), __( 'year', 'formidable' ), __( 'years', 'formidable' ), __( 'yearly', 'formidable-views' ) );
		...
		if ( ! $repeat_period || ! is_numeric( strtotime( $repeat_period ) ) ) {
			return;
		}
	...
	public static function get_listing_page_content( $view, $atts ) {
		...
		self::add_inline_custom_css( $view );
	...
	private static function get_detail_page_content( $view, $atts ) {
		...
		self::add_inline_custom_css( $view, 'detail' );
	...
	/**
	 * Enqueue inline custom CSS for a view's listing or detail page.
	 *
	 * @since 5.10
	 *
	 * @param object $view
	 * @param string $page_type Either 'listing' or 'detail'.
	 * @return void
	 */
	private static function add_inline_custom_css( $view, $page_type = 'listing' ) {
		if ( 'detail' === $page_type ) {
			$custom_css = $view->frm_detail_page_custom_css;
		} elseif ( ! empty( $view->frm_custom_css ) ) {
			$custom_css = $view->frm_custom_css;
		} else {
			$custom_css = $view->frm_listing_page_custom_css;
		}
		...
		FrmViewsInlineStyleController::get_instance()->set_style( 'formidable-views-' . $page_type, $custom_css );
	}
	...
	private static function apply_frm_search_to_view( $view ) {
		...
		if ( $search_view_ids && ! in_array( $view->ID, $search_view_ids ) ) {
			return false;
		}
	...
	private static function check_unique_filters( $view, &$where ) {
		...
		foreach ( $view->frm_where as $i => $filter_field ) {
			...
			if ( is_numeric( $view->frm_where[ $i ] ) ) {
				...
				if ( in_array( $view->frm_where[ $i ], array( 'id', 'item_key' ), true ) ) {
					continue;
				}
	...
	*/
	...
	/**
	 * @param int $view_id
	 * @return void active preview filter value. Either '0', 'limited', or '1'.
	 */
	...
	private static function maybe_add_cat_query( &$where ) {
		...
		FrmAppHelper::unserialize_or_decode( $categories );
