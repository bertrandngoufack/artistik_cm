( function() {
	...
	const FrmSurveys = {
		...
		initHooks: function() {
			...
			hooks.addAction( 'frm_after_bulk_edit_opts', this.maybeHideUploadImage );
		},
		...
		/**
		 * Maybe hide the image upload button if the field display format is "Simple".
		 *
		 * @param {string} fieldId The ID of the field.
		 */
		maybeHideUploadImage: function( fieldId ) {
			const useImagesInButtonsCheckbox = document.getElementById( `frm_use_images_in_buttons_${ fieldId }` );
			if ( ! useImagesInButtonsCheckbox ) {
				return;
			}

			const useImagesInButtons = useImagesInButtonsCheckbox.checked;
			const displayFormatContainer = document.getElementById( `frm_display_format_${ fieldId }_container` );
			if ( ! displayFormatContainer ) {
				return;
			}

			const displayFormat = displayFormatContainer.querySelector( 'input.frm_radio_display_format:checked' );
			if ( ! displayFormat ) {
				return;
			}

			const displayFormatValue = displayFormat.value;

			if ( useImagesInButtons && 0 === Number( displayFormatValue ) ) {
				document.querySelectorAll( `.field_${ fieldId }_image_id` ).forEach(
					imagePreviewWrapper => imagePreviewWrapper.classList.add( 'frm_hidden' )
				);
			}
		},
