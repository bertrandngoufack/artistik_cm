			...
			<span>
				<select <?php echo esc_attr( $select_name_attr ); ?> class="frm-ranking-position" id="<?php echo esc_attr( $field_select_id ); ?>">
					...
					<?php for ( $i = 1; $i <= $options_count;  $i++ ) : ?>
						<option <?php echo $active_selection === $i ? 'selected' : ''; ?> <?php echo ! empty( $field['value']['order'][ $i - 1 ] ) && $active_selection !== $i ? 'disabled' : ''; ?> value="<?php echo (int) $i; ?>" > <?php echo (int) $i; ?> </option>
					<?php endfor; ?>
				</select>
