...
class FrmStrpConnectHelper {
	...
	private static function post_to_connect_server( $action, $additional_body = array() ) {
		$body    = array(
			...
			'frm_strp_connect_mode'   => FrmStrpAppHelper::active_mode(),
		);
	...
	private static function get_active_mode_option_name_suffix( $mode = 'auto' ) {
		...
		return '_' . FrmStrpAppHelper::active_mode();
	}
	...
	private static function set_stripe_details_as_submitted_and_clear_legacy_keys( $mode ) {
		...
		if ( $updated_settings ) {
			update_option( 'frm_strp_options', $settings, false );
		}
	}

	/**
	 * Dismiss any inbox messages to use Stripe Connect after successfully connecting to Stripe Connect.
	 *
	 * @return void
	 */
	private static function dismiss_inbox_message_to_use_stripe_connect() {
		...
		// Dismiss the old message if it is still set.
		// This message was introduced with v2.04 when Stripe Connect was added.
		$key = 'use_strp_connect';
		$message->dismiss( $key );

		// Dismiss the newer deprecation message.
		// This message was introduced in v3.1.5 when a new deprecated message was added.
		// As of June 2024, Stripe will no longer support API keys.
	...
	private static function render_settings() {
		...
		foreach ( $modes as $mode ) {
			...
			ob_start();
			require FrmStrpAppHelper::plugin_path() . '/views/settings/connect.php';
	...
	private static function get_mode_value_from_post() {
		...
		if ( empty( $_POST ) || ! array_key_exists( 'testMode', $_POST ) ) {
			return FrmStrpAppHelper::active_mode();
		}
	...
	private static function register_settings_scripts() {
		$version = FrmStrpVersionCheck::get_version();
		wp_register_script( 'formidable_stripe_settings', FrmStrpAppHelper::plugin_url() . '/js/connect_settings.js', array(), $version, true );
	...
	private static function post_with_authenticated_body( $action, $additional_body = array() ) {
		...
		if ( is_string( $response ) ) {
			...
			FrmTransLog::log_message( 'Unable to complete call to Stripe Connect service: ' . $response );
		} else {
	...
	public static function run_new_charge( $new_charge ) {
		$data    = self::post_with_authenticated_body( 'run_new_charge', compact( 'new_charge' ) );
		$success = false !== $data;
		if ( ! $success ) {
			return false;
		}
	...
	public static function cancel_subscription( $sub_id, $customer_id = false ) {
		$cancel_at_period_end = FrmStrpSubscriptionHelper::should_cancel_at_period_end();
	...
	public static function can_by_captured( $charge_id ) {
		$capture = self::post_with_authenticated_body( 'can_by_captured', compact( 'charge_id' ) );
		return is_object( $capture ) && ! empty( $capture->capture );
	}
	...
	public static function capture_charge( $charge_id ) {
		if ( 0 === strpos( $charge_id, 'pi_' ) ) {
			return self::capture_intent( $charge_id, array() );
		}
		$data = self::post_with_authenticated_body( 'capture_charge', compact( 'charge_id' ) );
		return false !== $data;
	}
	...
	public static function get_cards( $user_id ) {
		$meta_name   = FrmStrpAppHelper::get_customer_id_meta_name();
		$customer_id = get_user_meta( $user_id, $meta_name, true );
		$data        = self::post_with_authenticated_body( 'get_cards', compact( 'customer_id' ) );

		if ( false === $data || ! isset( $data->cards ) ) {
			return array();
		}

		return self::process_card_object_back_into_expected_array_format( $data->cards );
	}
	 ...
	 * @return array
	 */
	private static function process_card_object_back_into_expected_array_format( $raw_array ) {
		$cards = array();
		foreach ( $raw_array as $card ) {
			$cards[ $card->card->id ] = array(
				'card'    => $card->card,
	...
	public static function delete_card( $customer_id, $card_id ) {
		$deleted = self::post_with_authenticated_body( 'delete_card', compact( 'customer_id', 'card_id' ) );
		return array(
			'success' => false !== $deleted,
			'error'   => false === $deleted ? __( 'Unable to delete card', 'formidable-stripe' ) : '',
		);
	}
	...
	public static function confirm_intent( $intent_id, $data ) {
		$data    = self::post_with_authenticated_body( 'confirm_intent', compact( 'intent_id', 'data' ) );
		$success = false !== $data;
		if ( ! $success ) {
			return false;
		}
	...
	public static function capture_intent( $intent_id, $data ) {
		$data    = self::post_with_authenticated_body( 'capture_intent', compact( 'intent_id', 'data' ) );
		$success = false !== $data;
		if ( ! $success ) {
			return false;
		}
