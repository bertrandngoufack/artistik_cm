...
class FrmTransActionsController {
	 ...
	 * @return void
	 */
	public static function before_update_form_settings( $form_id ) {
		...
		// Get published all payment actions
		$payment_actions = self::get_actions_for_form( $form_id );
		foreach ( $payment_actions as $payment_action ) {
			if ( ! self::should_add_payment_field( $payment_action->post_content ) ) {
				continue;
			}
			if ( ! $credit_card_field_id ) {
				$credit_card_field_id = self::create_payment_field( $form_id );
			}
			$payment_action->post_content['credit_card'] = $credit_card_field_id;

			$action_control = FrmFormActionsController::get_form_actions( 'payment' );

			$action_control->save_settings( $payment_action );
		}
	}
	 ...
	 * @return int
	 */
	private static function create_payment_field( $form_id ) {
		$new_values         = FrmFieldsHelper::setup_new_vars( 'credit_card', $form_id );
		$new_values['name'] = __( 'Payment', 'formidable' );
	 ...
	 * @return bool
	 */
	private static function should_add_payment_field( $settings ) {
		return array_intersect( $settings['gateway'], array( 'square', 'stripe', 'authnet_aim' ) );
	}
}
