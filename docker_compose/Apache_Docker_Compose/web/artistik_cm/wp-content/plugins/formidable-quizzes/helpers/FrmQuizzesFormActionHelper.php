...
class FrmQuizzesFormActionHelper {
	...
	/**
	 * Gets scored quiz action from form.
	 *
	...
	public static function get_quiz_action_from_form( $form_id, $active = false ) {
		...
		if ( $active && ! self::get_setting_value( $action, 'enable' ) ) {
			// Treat quiz with no questions as inactive.
			return null;
		}
		...
		if ( $active ) {
			 ...
			 * @param int     $form_id
			 */
			$action = apply_filters( 'frm_quizzes_scored_quiz_action', $action, $form_id );
		}
