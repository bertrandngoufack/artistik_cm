...
class FrmQuizzes {
	...
	public function get_scoring_for_field( $field_id ) {
		...
		foreach ( (array) $settings as $field_setting ) {
			self::preprocess_field_setting( $field_setting );
	...
	/**
	 * Preprocesses quiz field setting.
	 *
	 * @param array $field_setting Quiz field setting.
	 */
	private function preprocess_field_setting( &$field_setting ) {
		$field_setting = (array) $field_setting;
		if ( ! empty( $field_setting['scores'] ) ) {
			$field_setting['scores'] = array_map( 'floatval', $field_setting['scores'] );
		}
	}
	...
	private function calculate_action_score() {
		...
		$total_score = $this->get_updated_total_score_based_on_entry( $total_score, $quiz_action );

		if ( false === $total_score ) {
			return $total_score;
		}

		$this->maybe_update_total_score_based_on_child_entries( $total_score, $quiz_action );
	...
	/**
	 * Returns updated total score based on entry values.
	 *
	 * @since 3.1.3
	 *
	 * @param float  $total_score
	 * @param object $quiz_action
	 * @param int    $entry_id
	 * @return float|false
	 */
	private function get_updated_total_score_based_on_entry( $total_score, $quiz_action, $entry_id = 0 ) {
		if ( ! $entry_id ) {
			$entry_id = $this->entry_id;
		}

		$entry_values = new FrmProEntryValues( $entry_id );
	...
	/**
	 * Updates total score based on child entry values.
	 *
	 * @since 3.1.3
	 *
	 * @param float  $total_score
	 ...
	 * @return void
	 */
	private function maybe_update_total_score_based_on_child_entries( &$total_score, $quiz_action ) {
		$child_entry_ids = FrmDb::get_col( 'frm_items', array( 'parent_item_id' => $this->entry_id ) );

		foreach ( $child_entry_ids as $child_entry_id ) {
			$total_score = $this->get_updated_total_score_based_on_entry( $total_score, $quiz_action, $child_entry_id ); // @phpstan-ignore-line
		}
	}
	...
	public function check_field( $field_value ) {
		...
		if ( $this->entry->parent_item_id ) {
			$this->entry = FrmEntry::getOne( $this->entry->parent_item_id );
		}
	...
	protected function check_field_choice_with_adv_scoring( $value ) {
		...
		foreach ( $value as $val ) {
			if ( isset( $scores[ $val ] ) ) {
				$score = floatval( $scores[ $val ]['score'] );
				if ( $score > 0 ) {
	...
	protected function check_field_text( $field_value ) {
		...
		$field        = $this->field;
		$corrects     = $corrects_raw ? array_map( array( $this, 'preprocess_check_value' ), $corrects_raw, compact( 'field' ) ) : array( '' );
	...
	protected function check_text_answer_equal( $value, $corrects ) {
		return $this->check_text_answer_with_numeric_operator( $value, $corrects );
	}

	/**
	 * Checks if the user answer less than the correct answer.
	 *
	 * @since 2.0
	 * @param array|string $value    The user answer.
	 * @param array        $corrects The correct answers.
	 ...
	 * @return bool
	 */
	protected function check_text_answer_less_than( $value, $corrects ) {
		return $this->check_text_answer_with_numeric_operator( $value, $corrects, '<' );
	}

	/**
	 * Checks if the user answer greater than the correct answer.
	 *
	 * @since 2.0
	 * @param array|string $value    The user answer.
	 * @param array        $corrects The correct answers.
	 ...
	 * @return bool
	 */
	protected function check_text_answer_greater_than( $value, $corrects ) {
		return $this->check_text_answer_with_numeric_operator( $value, $corrects, '>' );
	}
	 ...
	 * @param array|string $value    The user answer.
	 * @param array        $corrects The correct answers.
	 * @param string       $operator The operator, supports `=`, `<`, `>`.
	 *
	 * @return bool
	 */
	protected function check_text_answer_with_numeric_operator( $value, $corrects, $operator = '=' ) {
		...
		if ( is_numeric( $value ) && is_numeric( $corrects ) ) {
			...
			return $this->compare_two_values( floatval( $value ), floatval( $corrects ), $operator );
		}

		return $this->compare_two_values( $value, $corrects, $operator );
	}
	 ...
	 * @param string $operator Comparison operator. Supports `=`, `<`, `>`.
	 *
	 * @return bool
	 */
	protected function compare_two_values( $value1, $value2, $operator = '=' ) {
		switch ( $operator ) {
			case '>':
