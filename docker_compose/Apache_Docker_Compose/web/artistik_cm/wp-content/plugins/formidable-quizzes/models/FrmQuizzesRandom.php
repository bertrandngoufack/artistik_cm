...
class FrmQuizzesRandom {
	...
	public function change_fields_order( $fields ) {
		 ...
		 */
		$fields = $this->maybe_randomize_questions_and_options( $fields );

		return $fields;
	}

	/**
	 * Randomizes the questions and options.
	 *
	 * @since 3.1.7
	 *
	 * @param array $fields Fields list.
	 * @return array
	 */
	private function maybe_randomize_questions_and_options( $fields ) {
		$form = FrmForm::getOne( $this->form_id );
		if ( isset( $form->options['quiz'] ) ) {
			$quiz_options = $form->options['quiz'];
		} else {
			$quiz_action  = FrmQuizzesFormActionHelper::get_quiz_action_from_form( $this->form_id, true );
			$quiz_options = $quiz_action ? $quiz_action->post_content : array();
		}
		if ( ! empty( $quiz_options['random_options'] ) ) {
			$this->sort_fields_options_randomly( $fields );
		}
		if ( empty( $quiz_options['random_questions'] ) ) {
			return $fields;
		}
	...
	protected function get_randomized_fields( $fields ) {
		$submit_field  = self::get_submit_field_and_remove_from_list( $fields );
	...
	/**
	 * Gets the submit field and removes it from the list.
	 *
	 * @since 3.1.5
	 *
	 * @param array $fields Fields list.
	 * @return object|false
	 */
	protected function get_submit_field_and_remove_from_list( &$fields ) {
		...
		for ( $i = count( $keys ) - 1; $i >= 0; $i-- ) {
			if ( FrmSubmitHelper::FIELD_TYPE === FrmField::get_field_type( $fields[ $keys[ $i ] ] ) ) {
				$submit_field = $fields[ $keys[ $i ] ];
				unset( $fields[ $keys[ $i ] ] );
				return $submit_field;
			}
		}
