...
class FrmQuizzesAction extends FrmFormAction {
	...
	private function print_settings( $form_action ) {
		$this->set_form_object( $form_action );
		$settings = $this->get_common_settings();
		include FrmQuizzesAppController::path() . '/views/shared/settings.php';
	 ...
	 * @since 3.1.7
	 *
	 * @param object $action Form action post object.
	 * @return void
	 */
	protected function set_form_object( $action ) {
		$form_id = $this->get_form_id_from_action( $action );
		if ( empty( $form_id ) ) {
			return;
		}
	...
	protected function get_setting_value( $key ) {
		...
		return FrmQuizzesFormActionHelper::get_setting_value( $quiz_action, $key );
	}
	...
	protected function get_fields_for_settings( $form_action, $quiz_type = 'scored' ) {
		...
		$form_ids = is_callable( 'FrmProFormsHelper::get_embedded_form_ids' ) ? FrmProFormsHelper::get_embedded_form_ids( $form_id ) : array();
