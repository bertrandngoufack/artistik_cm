...
class FrmQuizzesEntryFormatter extends FrmProEntryFormatter {
	...
	protected function show_scored_user_answers( $field_value, $result ) {
		...
		$field         = $field_value->get_field();
		$field_options = ! empty( $field->options ) ? $field->options : array();
		foreach ( (array) $array_values as $value ) {
			if ( $field_options ) {
				$value = self::get_option_label( $value, $field_options );
			}
	...
	protected function add_correct_answers( $field, $result ) {
		...
		$correct = $this->format_correct_answers( $result, $field->options );
	 ...
	 * @return string
	 */
	private static function get_option_label( $value, $options ) {
		$index = array_search( $value, array_column( $options, 'value' ), true );
		if ( false !== $index ) {
	...
	/**
	 * Get the correct answers labels.
	 *
	 * @since 3.1.7
	 *
	 * @param array $corrects_raw
	 * @param array $options
	 * @return array
	 */
	private static function get_corrects_raw_labels( $corrects_raw, $options ) {
		if ( ! $options ) {
			return $corrects_raw;
		}

		if ( is_array( $corrects_raw ) ) {
			foreach ( $corrects_raw as $key => $correct ) {
				$corrects_raw[ $key ] = self::get_option_label( $correct, $options );
			}
			return $corrects_raw;
		}
		return self::get_option_label( $corrects_raw, $options );
	}
	 ...
	 * @return string
	 */
	protected function format_correct_answers( $result, $options = array() ) {
		$result['corrects_raw'] = self::get_corrects_raw_labels( $result['corrects_raw'], $options );
		...
		switch ( $result['compare'] ) {
			case 'greater_than':
				// Translators: correct answer.
				return sprintf( __( 'Greater than %s', 'formidable-quizzes' ), $correct );

			case 'less_than':
				// Translators: correct answer.
				return sprintf( __( 'Less than %s', 'formidable-quizzes' ), $correct );
	...
	public function get_formatted_entry_values() {
		$content = parent::get_formatted_entry_values();
		if ( $content ) {
			ob_start();
