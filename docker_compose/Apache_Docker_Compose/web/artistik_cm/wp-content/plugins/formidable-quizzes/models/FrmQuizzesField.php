...
class FrmQuizzesField extends FrmFieldHidden {
	...
	protected function prepare_display_value( $value, $atts ) {
		...
		$outcome           = $this->maybe_get_outcome_display_value( $value, $atts, $outcome_displayed );
		if ( is_string( $outcome ) ) {
			if ( ! in_array( $atts['show'], array( 'score', 'percent' ), true ) ) {
				return $outcome;
			}
			...
			if ( ! empty( $atts['outcome'] ) ) {
				$outcome_displayed = FrmFormAction::get_single_action_type( $atts['outcome'], FrmQuizzesFormActionHelper::$outcome_action_name );
				if ( ! $outcome_displayed instanceof WP_Post ) {
					return '';
				}
			}
			$outcome_helper     = new FrmQuizzesOutcomeHelper( $outcome_displayed, $entry );
			$outcome_conditions = $outcome_helper->filter_conditions( $outcome_displayed->post_content );
			$value              = $outcome_helper->get_outcome_score( $outcome_conditions );
			$max_score          = count( $outcome_conditions );
		}
		...
		switch ( $atts['show'] ) {
			...
			case 'percentile':
				$value = round( $this->get_percentile( $value ), 2 );
				break;
	...
	/**
	 * @param float|int|string $given_score
	 * @return float
	 */
	private function get_percentile( $given_score ) {
		$scores = FrmDb::get_col(
			'frm_item_metas',
			array(
				'field_id' => is_array( $this->field ) ? $this->field['id'] : $this->field->id,
			),
			'meta_value'
		);

		$given_score = (float) $given_score;
		$scores      = array_map( 'floatval', $scores );

		// Step 1: Sort the array in ascending order.
		sort( $scores );

		// Step 2: Find the rank of the given score.
		$rank = ( (int) array_search( $given_score, $scores, true ) ) + 1;
	 ...
	 *
	 * @since 3.1.5 Added the third parameter, $outcome_displayed.
	 *
	 * @param mixed $value
	 * @param array $atts {
	 *     @type string $show
	 ...
	 * }
	 * @param false $outcome_displayed
	 *
	 * @return string|false
	 */
	private function maybe_get_outcome_display_value( $value, $atts, &$outcome_displayed ) {
		if ( ! is_numeric( $value ) ) {
			return false;
		}
