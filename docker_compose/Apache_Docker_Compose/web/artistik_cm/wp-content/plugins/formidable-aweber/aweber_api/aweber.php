...
/**
 * FrmAWeberServiceProvider
 *
 * Provides specific AWeber information or implementing OAuth.
 * @uses FrmOAuthServiceProvider
 * @package
 * @version $id$
 */
class FrmAWeberServiceProvider implements FrmOAuthServiceProvider {
 ...
 * FrmAWeberAPIBase
 *
 * Base object that all FrmAWeberAPI objects inherit from.  Allows specific pieces
...
class FrmAWeberAPIBase {
     ...
     * @access public
     * @return FrmAWeberEntry or FrmAWeberCollection
     */
     ...
     * @param mixed $response   Data returned from a request to the FrmAWeberAPI
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new FrmAWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new FrmAWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * FrmAWeberAPI
 *
 * Creates a connection to the FrmAWeberAPI for a given consumer application.
 ...
 * @uses FrmAWeberAPIBase
 * @package
 * @version $id$
 */
class FrmAWeberAPI extends FrmAWeberAPIBase {
    ...
    public static function getDataFromAweberID($string) {
        list($consumerKey, $consumerSecret, $requestToken, $tokenSecret, $verifier) = FrmAWeberAPI::_parseAweberID($string);
        if (!$verifier) {
            return null;
        }
        $aweber = new FrmAweberAPI($consumerKey, $consumerSecret);
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new FrmAWeberServiceProvider();
            $adapter = new FrmOAuthApplication($serviceProvider);
     ...
     *      on the same page)
     * @return Object FrmAWeberCollection Object with the requested
     *     account data
     */
    public function getAccount($token=false, $secret=false) {
        if ($token && $secret) {
            $user = new FrmOAuthUser();
