...
class FrmAWeberEntry extends FrmAWeberResponse {
    ...
    /**
     * @var array Holds FrmAWeberCollection objects already instantiated, keyed by
    ...
    public function attrs() {
        ...
        if (!empty(FrmAWeberAPI::$_collectionMap[$this->type])) {
            foreach (FrmAWeberAPI::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param FrmAWeberEntry(List)   List to move Resource (this) too.
     * @return mixed FrmAWeberEntry(Resource) Resource created on List ($list)
    ...
    public function move($list, $last_followup_message_number_sent=NULL) {
        ...
        return new FrmAWeberEntry($resource_data, $url, $this->adapter);
    }
     ...
     * @access public
     * @throws FrmAWeberResourceNotImplemented
     * @return mixed
     */
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new FrmAWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new FrmAWeberResourceNotImplemented($this, $value);
    }
    ...
    public function findSubscribers($search_data) {
        ...
        return new FrmAWeberCollection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new FrmAWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    public function tags() {
        return $this->adapter->request( 'GET', $this->url . '/tags' );
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new FrmAWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes) {
        ...
        throw new FrmAWeberMethodNotImplemented($this);
    }

    /**
     * _getCollection 
     *
     * Returns the FrmAWeberCollection object representing the given
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new FrmAWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(FrmAWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, FrmAWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
