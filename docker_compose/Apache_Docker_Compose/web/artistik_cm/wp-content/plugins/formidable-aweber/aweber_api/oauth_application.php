 ...
 *
 * For example, a FrmOAuthServiceProvider would know the URLs necessary
 ...
 *
 * It should be noted that the methods enforced by the FrmOAuthServiceProvider
 * interface are made so that it can interact with our FrmOAuthApplication
 ...
 * @version $id$
 */
interface FrmOAuthServiceProvider {
 ...
 * FrmOAuthServiceProvider should have a complementary FrmOAuthApplication
 *
 * The FrmOAuthApplication class should contain any details on preparing
 ...
 * as a model.  An FrmOAuthApplication built to work with a different service
 ...
 * @version $id$
 */
class FrmOAuthApplication implements FrmAWeberOAuthAdapter {
    public $debug = false;
    ...
    /**
     * @var FrmOAuthUser User currently interacting with the service provider
     */
    public $user = false;

    // Data binding this FrmOAuthApplication to the consumer application it is acting
    // as a proxy for
     ...
     * __construct
     *
     * Create a new FrmOAuthApplication, based on an FrmOAuthServiceProvider
     * @access public
     * @return void
     */
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'FrmOAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid FrmOAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new FrmOAuthUser();
    }
    ...
    public function request($method, $uri, $data = array(), $options = array()) {
        ...
        if (empty($options['allow_empty']) && !isset($data)) {
            throw new FrmAWeberResponseError($uri);
        }
        return $data;
    }
    ...
    public function getAccessToken() {
        ...
        if (empty($data['oauth_token'])) {
            throw new FrmAWeberOAuthDataMissing('oauth_token');
        }
     ...
     * @access public
     * @throws FrmAWeberOAuthException
     * @return void
     */
    public function parseAsError($response) {
        if (!empty($response['error'])) {
            throw new FrmAWeberOAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields) {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new FrmAWeberOAuthDataMissing($field);
            }
        }
    }
    ...
    protected function encode( $data ) {
        return rawurlencode( FrmAppHelper::maybe_utf8_encode( $data ) );
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new FrmAWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            throw new FrmAWeberAPIException($data['error'], $url);
        }
    ...
    public function parseResponse($resp) {
		...
		switch ( $this->format ) {
			...
			default:
				...
				if ( empty( $data['oauth_token'] ) ) {
					$pos = strpos( $resp->body, 'oauth_token=' );
					if ( false !== $pos ) {
						$pos                += strlen( 'oauth_token=' );
						$end_pos             = strpos( $resp->body, '&oauth_token_secret=', $pos );
						$length              = $end_pos - $pos;
						$data['oauth_token'] = substr( $resp->body, $pos, $length );
					}
				}
...
class FrmOAuthUser {
     ...
     * state for a given FrmOAuthUser
     * @access public
     * @return void
     */
