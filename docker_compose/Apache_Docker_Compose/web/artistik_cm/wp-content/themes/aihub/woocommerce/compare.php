...
$table_text = get_option( 'yith_woocompare_table_text', esc_html__( 'Compare products', 'aihub' ) );
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo get_template_directory_uri( ) . '/assets/css/liquid-yith-compare.min.css' ?>" type="text/css" />
...
<h1>
    <?php echo apply_filters ( 'wpml_translate_single_string', $table_text, 'Plugins', 'plugin_yit_compare_table_text' ); ?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'aihub' ) ?></a><?php endif; ?>
</h1>
...
<div id="yith-woocompare" class="woocommerce">
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'aihub' ) ?></td>
        </tr>
    ...
    <?php else : ?>
        <tr class="remove">
            ...
            foreach( $products as $product_id => $product ) :
                ...
                <td class="<?php echo sanitize_html_class( $product_class ); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php esc_html_e( 'Remove', 'aihub' ) ?> <span class="remove">x</span></a>
                </td>
                <?php
                ++$index;
            endforeach;
        ...
        <?php foreach ( $fields as $field => $name ) : ?>
            ...
            <tr class="<?php echo sanitize_html_class( $field ) ?>">

                <th>
                    <?php if ( $field != 'image' ) echo wp_kses_post( $name ); ?>
                </th>
                ...
                foreach( $products as $product_id => $product ) :
                    $product_class = 'product_' . $product_id; ?>
                    <td class="<?php echo sanitize_html_class( $product_class ); ?>"><?php
                        switch( $field ) {
        ...
        <?php if ( $repeat_price == 'yes' && isset( $fields['price'] ) ) : ?>
            <tr class="price repeated">
                ...
                foreach( $products as $product_id => $product ) :
                    $product_class = 'product_' . $product_id ?>
                    <td class="<?php echo sanitize_html_class( $product_class ); ?>"><?php echo wp_kses_post( $product->fields['price'] ) ?></td>
                    <?php
                    ++$index;
                endforeach; ?>
