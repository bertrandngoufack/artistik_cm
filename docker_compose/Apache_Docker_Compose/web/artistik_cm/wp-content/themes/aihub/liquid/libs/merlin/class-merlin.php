...
class Merlin {
	...
	public function admin_page() {
		...
		wp_add_inline_style( 'merlin', '#qm-icon-container,#query-monitor-main{display:none}' ); // Hide Query Monitor plugin.
		...
		wp_enqueue_script( 'liquid-admin', liquid()->load_assets( 'js/liquid-admin.min.js' ), array( 'jquery', 'underscore' ), true, true );
				...
				<div class="merlin__content--license-key">
					...
					<div class="lqd-dsd-box lqd-dsd-box-solid lqd-dsd-register-box">
						...
						<div class="lqd-dsd-box-head">
							...
							<?php 
								if ( 'valid' == get_option( 'aihub_purchase_code_status', false ) ) {
	...
	protected function plugins() {
		...
		<div class="merlin__content--transition">
			...
			<?php 
				// Redirect if there is no available plugin.
				// if ( $count === 0 ) {
				// 	wp_redirect(admin_url('admin.php?page=liquid-setup&step=content'));
				// }
			?>
	...
	function elementor_update_options() {
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'liquid-header', 'liquid-footer', 'liquid-portfolio', 'liquid-mega-menu', 'ld-product-layout', 'ld-product-sizeguide', 'liquid-sticky-atc', 'liquid-archives' ] );
	}
