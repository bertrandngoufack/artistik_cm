...
function liquid_add_custom_general_fields() {
  ...
  echo '<div class="options_group">';
	...
	woocommerce_wp_textarea_input(
		array(
			'id'          => '_custom_label',
			'label'       => esc_html__( 'Custom label', 'aihub' ),
			'desc_tip'    => 'true',
			'description' => esc_html__( 'Input a custom label for the product', 'aihub' )
		)
	);
	echo '</div>';
...
function liquid_add_custom_general_fields_save( $post_id ){
	...
	// Custom button label
	$woo_custom_label = wp_kses( $_POST['_custom_label'], 'lqd_post' );
	if( isset( $woo_custom_label ) ) {
...
?>
	<tr class="form-field">
		<th scope="row">
			<label for="term-background-color"><?php esc_html_e( 'Background Color', 'aihub' ); ?></label>
			<td>
				...
				<p class="description"><?php esc_html_e( 'Select a background for product category label', 'aihub' ); ?></p>
			</td>
		</th>
	</tr>
<?php
...
function liquid_create_background_meta_field() {
	...
	<div class="form-field term-background-wrap">
		...
		<p><?php esc_html_e( 'Select a background for product category label', 'aihub' ); ?></p>
	</div>
<?php }