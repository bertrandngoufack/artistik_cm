...
class Liquid_Register {
	...
	function __construct( $config = array(), $strings = array() ) {
		$config = wp_parse_args( $config, array(
			'remote_api_url' => 'https://api.liquid-themes.com/aihub',
			'theme_slug'     => 'aihub',
			'version'        => '',
	...
	public function init_hooks() {
		if ( ! empty( $this->site_url ) && empty( $this->is_check_domain() ) ) {
			if ( !$this->check_wpml() ) {
				$this->check_domain();
			}
		}
	...
	function admin_error() {
		echo '<div class="error"><p>' . sprintf( wp_kses( __( 'The %s theme needs to be registered. %sRegister Now%s', 'aihub' ), 'a' ), 'AIHub', '<a href="' . admin_url( 'admin.php?page=liquid') . '">' , '</a>' ) . '</p></div>';
	}
	...
	function messages() {
		...
		if ( isset($_GET['page']) && ($_GET['page'] == 'liquid') ){
			if ( $status === false || empty( $status ) || $status != 'valid' ) {
				$tag = 'h4';
				$message = '<'.$tag.'>Activate AIHub</'.$tag.'><p>Go to <a href="https://portal.liquid-themes.com/">portal.liquid-themes.com</a> and create your Liquid account before activating your theme</p>';
	...
	function form() {
		...
		<form action="https://portal.liquid-themes.com/license/activate" method="GET" target="_blank" class="lqd-dsd-register-form">
			...
			<input type="hidden" name="envato_item_id" value="<?php echo esc_attr( $this->envato_item_id ); ?>" />
			...
			<button type="submit">
				<?php esc_html_e( 'Connect to Liquid Portal', 'aihub' ) ?>
			</button>
		</form>
		<?php
		else:
		...
		<div class="lqd-dsd-box-foot">
			<a href="https://portal.liquid-themes.com/" target="_blank"><?php esc_html_e( 'Manage Licenses in Liquid Portal', 'aihub' ); ?></a>
			...
			<?php settings_fields( $this->theme_slug . '-license' ); ?>
				...
				<button type="submit" style="background-color: #dc2626;">
					<?php esc_html_e( 'Revoke License', 'aihub' ) ?>
				</button>
			</form>
		</div>
		<?php
	...
	function sanitize_license( $new ) {
		$old = get_option( 'aihub_purchase_code' );
	...
	function check_license() {
		...
		if ( $_GET['itemId'] != $this->envato_item_id  ) {
			return;
		}
		...
		if ( !defined( 'WP_HOME' ) || !defined( 'WP_SITEURL' ) ) {
			if ( !get_option( $this->theme_slug . '_purchase_code_domain' ) ){
				...
				if (
					...
					$message = '<h3>Action Required: Reactivate Your AIHub Theme License</h3>
						<p>We’ve detected that your site is now on a new domain, but your AIHub Theme license is still registered to the old one. To restore full functionality, transfer your license to the current domain.</p>
	...
	function is_check_domain(){
		return get_option( $this->theme_slug . '_is_check_domain', '' );
	}
	...
	function check_revoke() {
		if (
			( isset( $_GET['liquid_license_revoke'] ) && ! empty( $_GET['liquid_license_revoke'] ) ) &&
			( isset( $_GET['itemId'] ) && $_GET['itemId'] == $this->envato_item_id ) &&
			( isset( $_GET['license_code'] ) && $_GET['license_code'] == trim( get_option( $this->theme_slug . '_purchase_code' ) ) )
		) {
	...
	function frontend_message_style() {
		...
		wp_add_inline_style( 'liquid-theme', $style );
