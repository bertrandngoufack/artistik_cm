...
function liquid_attributes_post( $attributes ) {
	...
	if ( ! empty( $post ) ) {
		...
		if( !class_exists( 'Liquid_Addons' ) ) {
			$attributes['class'] = join( ' ', get_post_class( array( $attributes['class'], 'lqd-lp lqd-lp-style-22 lqd-lp-title-34 relative' ) ) );
		}
		else {
...
function liquid_attributes_entry_title( $attributes ) {
	...
	if( !class_exists( 'Liquid_Addons' ) ) {
		$attributes['class']    = isset( $attributes['class'] ) ? $attributes['class'] . ' lqd-lp-title mt-56 mb-16 h5 lh-125' : '';
	}
	else {
...
function liquid_attributes_entry_published( $attributes ) {
	...
	$attributes['title']    = get_the_time( esc_html_x( 'l, F j, Y, g:i a', 'post time format', 'aihub' ) );
...
function liquid_attributes_entry_summary( $attributes ) {
	...
	if( !class_exists( 'Liquid_Addons' ) ) {
		$attributes['class'] = 'entry-summary lqd-lp-excerpt mt-16 mb-16';
	}
	else {
...
function liquid_post_thumbnail() {
	...
	echo '<div class="lqd-lp-img overflow-hidden rounded-4 mt-24 mb-32">';
		the_post_thumbnail();
	echo '</div>';
}
...
function liquid_get_post_terms( $args ) {
	...
	$defaults = array(
		...
		'sep'        => esc_html_x( ', ', 'taxonomy terms separator', 'aihub' )
	);
	$args = wp_parse_args( $args, $defaults );

	$classnames = 'lqd-lp-cat flex flex-wrap items-center list-none p-0 m-0 relative z-2';

	if ( $args['solid'] ) {
		$classnames .= 'lqd-lp-cat-shaped lqd-lp-cat-solid';
	}

	$terms = get_the_term_list( $args['post_id'], $args['taxonomy'], '<ul class="'.esc_attr($classnames).'"><li class="circle">', '</li><li class="circle">', '</li></ul>' );
...
function liquid_post_title_description() {
	...
	elseif ( is_search() )
		$title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'aihub' ), get_search_query() );
	...
	elseif ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'aihub' ) );

	elseif ( get_query_var( 'minute' ) )
		$title = sprintf( esc_html__( 'Minute %s', 'aihub' ), get_the_time( esc_html_x( 'i', 'minute archives time format', 'aihub' ) ) );

	elseif ( get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g a', 'hour archives time format', 'aihub' ) );

	elseif ( is_day() )
		$title = get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'aihub' ) );

	elseif ( get_query_var( 'w' ) )
		$title = sprintf( esc_html__( 'Week %1$s of %2$s', 'aihub' ), get_the_time( esc_html_x( 'W', 'weekly archives date format', 'aihub' ) ), get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'aihub' ) ) );

	elseif ( is_month() )
		$title = single_month_title( ' ', false );

	elseif ( is_year() )
		$title = get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'aihub' ) );

	elseif ( is_archive() )
		$title = hesc_html__( 'Archives', 'aihub' );
