<?php
/**
 * Audit Complet du Projet KISSAI SCHOOL - LyCol
 * Vérification de tous les aspects : routes, CRUD, conformité, cohérence
 * Port cible : 8080
 */

class AuditCompletProjet {
    ...
    public function __construct() {
        echo "🔍 AUDIT COMPLET DU PROJET KISSAI SCHOOL - LyCol\n";
        echo "================================================\n";
        echo "Port cible : 8080\n";
    ...
    public function run() {
        ...
        $this->testPortConfiguration();
        $this->testLinksAndReferences();
        $this->generateReport();
    }
    ...
    private function testServerConnection() {
        ...
        if ($response['status'] === 200) {
            ...
            $this->errors[] = "❌ Serveur non accessible sur port 8080 (Status: {$response['status']})";
    ...
    private function testDatabaseConnection() {
        ...
        try {
            ...
            $this->results['database'] = "✅ Base de données accessible ({$result['count']} licences)";
    ...
    private function testMainRoutes() {
        echo "3️⃣ Test des routes principales...\n";
        ...
        foreach ($routes as $route => $description) {
            ...
            if ($response['status'] === 200) {
                $this->results["route_{$route}"] = "✅ {$description} accessible";
                echo "   ✅ {$description}\n";
            } else {
    ...
    private function testAdminRoutes() {
        echo "4️⃣ Test des routes d'administration...\n";
        ...
        foreach ($adminRoutes as $route => $description) {
            $response = $this->makeRequest($route);
            if ($response['status'] === 200) {
                $this->results["admin_route_{$route}"] = "✅ {$description} accessible";
                echo "   ✅ {$description}\n";
            } else {
                $this->warnings[] = "⚠️ {$description} non accessible (Status: {$response['status']})";
                echo "   ⚠️ {$description} non accessible\n";
    ...
    private function testConfigurationModule() {
        ...
        if ($response['status'] === 200) {
            $this->results['config_module'] = '✅ Module configuration accessible';
        ...
        $postData = [
            'school_name' => 'KISSAI SCHOOL - Test',
    ...
    private function testLicenseModule() {
        ...
        if ($response['status'] === 200) {
            $this->results['license_module'] = '✅ Module licences accessible';
    ...
    private function testPortConfiguration() {
        echo "9️⃣ Vérification de la configuration du port...\n";
    ...
    private function testLinksAndReferences() {
        echo "🔟 Vérification des liens et références...\n";
        
        // Vérifier les liens dans les vues
        $viewFiles = [
            'app/Views/admin/configuration/index.php',
            'app/Views/admin/configuration/license.php',
            'app/Views/admin/configuration/appearance.php'
        ];
        ...
        foreach ($viewFiles as $file) {
            if (file_exists($file)) {
                $content = file_get_contents($file);
                if (strpos($content, '8080') !== false) {
                    echo "   ✅ {$file} contient des références au port 8080\n";
                } else {
                    $this->warnings[] = "⚠️ {$file} ne contient pas de références au port 8080";
                    echo "   ⚠️ {$file} ne contient pas de références au port 8080\n";
                }
            }
        }
    }
    ...
    private function makeRequest($url, $method = 'GET', $data = null) {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'AuditBot/1.0');
    ...
    private function generateReport() {
        ...
        echo "\n🎯 RECOMMANDATIONS:\n";
        echo "   1. Vérifier que tous les liens pointent vers le port 8080\n";
        echo "   2. S'assurer que la configuration est dans le fichier .env\n";
        echo "   3. Tester tous les formulaires CRUD\n";
