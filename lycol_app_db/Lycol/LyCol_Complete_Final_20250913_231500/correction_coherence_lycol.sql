-- =====================================================
-- SCRIPT DE CORRECTION DE COHÉRENCE - PROJET LYCOL
-- Date : 26 Août 2025
...
ALTER TABLE grades ADD CONSTRAINT IF NOT EXISTS chk_marks 
CHECK (marks_obtained >= 0 AND marks_obtained <= 20);

-- Validation des coefficients des matières
ALTER TABLE subjects ADD CONSTRAINT IF NOT EXISTS chk_coefficient 
CHECK (coefficient > 0 AND coefficient <= 4);

-- Validation des montants de paiement
ALTER TABLE payments ADD CONSTRAINT IF NOT EXISTS chk_amount 
CHECK (amount_paid > 0);

-- Validation des coefficients d'examens
ALTER TABLE exams ADD CONSTRAINT IF NOT EXISTS chk_exam_coefficient 
...
ALTER TABLE students ADD CONSTRAINT IF NOT EXISTS uk_student_matricule 
UNIQUE (matricule, academic_year);

-- Ajouter des contraintes pour les codes de classe uniques
ALTER TABLE classes ADD CONSTRAINT IF NOT EXISTS uk_class_code 
