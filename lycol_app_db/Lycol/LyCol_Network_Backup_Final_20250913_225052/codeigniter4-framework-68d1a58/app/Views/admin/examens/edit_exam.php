        ...
        <div class="column is-12">
            ...
            <nav class="breadcrumb" aria-label="breadcrumbs">
                <ul>
                    ...
                    <li class="is-active"><a href="#" aria-current="page">Modifier l'Examen</a></li>
                </ul>
            </nav>
            ...
            <div class="level">
                <div class="level-left">
                    <div class="level-item">
                        <h1 class="title">Modifier l'Examen</h1>
                    </div>
                </div>
            ...
            <!-- Form -->
            <div class="box">
                <form action="<?= base_url('admin/examens/exams/' . $exam['id'] . '/update') ?>" method="post">
                    <?= csrf_field() ?>
                        ...
                        <div class="control">
                            <div class="select <?= session('errors.exam_type') ? 'is-danger' : '' ?> is-fullwidth">
                                <select id="exam_type" name="exam_type" required>
                                    ...
                                    <option value="CONTINUOUS" <?= old('exam_type', $exam['exam_type']) == 'CONTINUOUS' ? 'selected' : '' ?>>Contrôle Continu</option>
                                    <option value="MIDTERM" <?= old('exam_type', $exam['exam_type']) == 'MIDTERM' ? 'selected' : '' ?>>Mi-trimestre</option>
                                    <option value="FINAL" <?= old('exam_type', $exam['exam_type']) == 'FINAL' ? 'selected' : '' ?>>Final</option>
                                    <option value="COMPETITIVE" <?= old('exam_type', $exam['exam_type']) == 'COMPETITIVE' ? 'selected' : '' ?>>Compétitif</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="field">
                                ...
                                <div class="control">
                                    <input class="input <?= session('errors.coefficient') ? 'is-danger' : '' ?>" 
                                           ...
                                           id="coefficient" 
                                           name="coefficient" 
                                           value="<?= old('coefficient', $exam['coefficient'] ?? 1.0) ?>" 
                                           min="0.1" 
                                           max="10" 
                                           step="0.1">
                                </div>
                        ...
                        <div class="control">
                            <div class="select is-fullwidth">
                                <select id="status" name="status">
                                    <option value="SCHEDULED" <?= old('status', $exam['status']) == 'SCHEDULED' ? 'selected' : '' ?>>Programmé</option>
                                    <option value="IN_PROGRESS" <?= old('status', $exam['status']) == 'IN_PROGRESS' ? 'selected' : '' ?>>En cours</option>
