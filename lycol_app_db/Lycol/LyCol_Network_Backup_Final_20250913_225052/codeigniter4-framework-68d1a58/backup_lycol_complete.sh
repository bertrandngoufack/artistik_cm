...
# Script de sauvegarde complet pour LyCol
# Usage: ./backup_lycol_complete.sh [backup_name]

echo "💾 SAUVEGARDE COMPLÈTE LYCOL"
echo "============================"

# Configuration
BACKUP_NAME=${1:-"lycol_complete"}
...
if [ -d "../${PROJECT_DIR}" ]; then
    tar -czf "$PROJECT_BACKUP" \
        ...
        --exclude="../${PROJECT_DIR}/backups_*" \
        --exclude="../${PROJECT_DIR}/test_*.php" \
        --exclude="../${PROJECT_DIR}/*.pdf" \
        --exclude="../${PROJECT_DIR}/*.sql" \
        --exclude="../${PROJECT_DIR}/*.md" \
        --exclude="../${PROJECT_DIR}/*.sh" \
        -C .. "$PROJECT_DIR"
...
SERVEUR:
Le serveur est configuré pour démarrer sur le port 8080:
...
echo "🔄 RESTAURATION DE LA SAUVEGARDE LYCOL"
echo "====================================="
...
echo ""
echo "🚀 Serveur actuel :"
echo "   Le serveur LyCol est démarré sur http://localhost:8080"
echo "   Pour arrêter: Ctrl+C dans le terminal du serveur"
